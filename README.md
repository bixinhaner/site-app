...
- 安装开始=“仍关联有效开站工单”的设备首次绑定（bind/rebind），安装完成=开站工单首次提交，上线=站点首次进入 `online_pending_activation`，激活=站点首次进入 `operational / maintenance`，SSV=SSV 工单首次 completed（均排除 VOIDED）；旧数据缺少站点状态审计时，会自动回退到开站工单时间。
- **仪表盘上线/激活口径统一（2026-03-15）**：
- 仪表盘“上线站点”现在统一按站点首次进入 `online_pending_activation` 统计；“激活站点”统一按站点首次进入 `operational / maintenance` 统计。
- 站点事件趋势图、仪表盘概况卡片、站点详情关键节点接口已复用同一套节点时间，避免不同页面看到的“上线/激活”数量和时间不一致。
- 为兼容历史老数据，如果某些旧站点缺少站点状态变更审计记录，系统会自动回退到开站工单的 `activated_at / completed_at` 时间，不会因为升级口径把老站点漏掉。
