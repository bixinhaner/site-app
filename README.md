...
- **站点付款规则（2026-03-22，2026-07-02 更新）**: Web 管理端 `系统配置 -> 站点付款规则` 支持“默认付款方案 + 分包商付款方案”。默认方案兜底所有站点；当站点通过“站点分组”分配了 `分包商 / subcontractor` 维度，且该分包商配置了独立方案时，系统会自动套用对应付款节点、单价和比例。每条规则都可以自己配：对应节点、是否付款、固定金额或比例、是否依赖“开站工单最终 approve”、warning 是否折减以及折减比例。
