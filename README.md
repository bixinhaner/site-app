...
│   ├── （说明）一次性测试/迁移脚本已从 backend 根目录清理，归档到 temp/backend_script_archive_20260326/
...
- **档案 PDF 导出语言修复（2026-03-26）**：
- 已修复“站点管理 -> 勘察档案 -> 档案详情”导出 PDF 不跟随右上角语言的问题。
- 现在导出接口会读取 `locale`，并按当前语言输出标题、页码、目录、字段列头、附件区等文案；切到 `English / Bahasa Indonesia` 后导出结果会同步切换。
- 三类档案（勘察 / 开站 / SSV）的 `PDF / ZIP` 导出文件名现在也统一跟随当前语言，不再固定中文前缀；前后端命名规则保持一致。
- **档案 PDF 字段与照片顺序统一（2026-03-26）**：
- 三类档案（勘察 / 开站 / SSV）导出时，检查项内容已按“管理系统档案详情”的字段顺序输出。
- 每个字段值后紧跟该字段关联照片，不再采用“先整块字段表、再整块照片墙”的分离展示。
- 历史数据里未绑定 `field_id` 的照片会在该检查项末尾按“照片 / 未关联字段”分组兜底展示，避免图片丢失。
- **开站档案变更历史展示补强（2026-04-01）**：
- `站点管理 -> 开站档案 -> 档案详情 -> 历史` 中，操作人现在有兜底显示：优先姓名，其次用户名，再次 `用户#ID`，避免空白。
- 变更明细不再只支持字段/照片路径；对附件等其它 JSON Patch 路径也会显示通用明细（如 `add /attachments/-`）。
- 首版本若没有可解析 diff，明细会显示 `new`，不再展示 `-`。
- **三类档案审核人与历史兜底统一（2026-04-01）**：
- 已把“勘察档案 / 开站档案 / SSV档案”统一到同一口径：列表页的“检查人/审核人”优先显示姓名，姓名为空时回退用户名，再回退 `用户#ID`。
- 三类档案详情中的“变更历史”也统一支持：操作人兜底显示、非字段/照片类的通用 diff 明细展示，以及首版本无差异时显示 `new`。
- **档案 PDF 字体跨平台显示修复（2026-03-26）**：
- 三类档案导出统一走同一套字体注册逻辑，优先选择可嵌入的 CJK TrueType/TTC 字体，不再让勘察档案走独立回退链路。
- 已增强字体筛选：要求同时覆盖基础拉丁字符和常见中文字符，避免误选仅英文字体后在不同阅读器触发替换。
- 兜底仍保留 `STSong-Light`，但只在系统确实找不到可用 TrueType/TTC 时才会触发；主流环境下会优先使用内嵌字体，减少“有的电脑字距异常、有的正常”的情况。
- **英文报告中文残留修复（2026-03-26）**：
- 三类档案（勘察 / 开站 / SSV）目录中的数量后缀已统一国际化；英文和印尼语不再出现 `（1项）` 这类中文后缀。
- 同步修复了相关残留硬编码：`未命名分类/检查项` 回退文案、扇区/小区补充说明、`缺失` 状态等，导出时会按当前语言输出。
- **开站档案术语统一（2026-03-26）**：
- 开站档案英文术语已统一为 `Site Installation Archive / Site Installation Archive Report`，不再使用 `Opening Archive` 表述。
- 印尼语已统一为 `Arsip Instalasi Situs / Laporan Arsip Instalasi Situs`。
- `web-admin` 相关文案（菜单、站点详情入口、错误提示、导出文件名、legacy 翻译桥接、付款规则里的开站工单描述）已同步更新，前后端命名保持一致。
- **工单审核台国际化补齐（2026-03-26）**：
- `web-admin -> Work Order Review Desk` 已补齐“检查项审核”区域的多语言词条：筛选项（复审/范围/距离）、一键 AI 检查按钮与下拉项、右上角审核统计标签与状态提示。
- 检查项名称里历史数据常见的后缀（如 `- 设备 xxx` / `- 扇区 xxx` / `- 小区 xxx`）现已按当前语言实时重渲染，不再在英文或印尼语界面中混入中文后缀。
- **设备级检查项后缀显示规则统一（2026-03-26）**：
- `web-admin -> Work Order Review Desk` 中，设备级检查项名称统一为“永远显示后缀”。
- 后缀仅基于设备位信息生成：优先 `cell_id`，其次 `sector_id_band`；不再使用 `equipment_sn`，避免与“范围”列 SN 信息重复。
- **工单审核台相似图详情跳转（2026-04-05）**：
- `web-admin -> 工单审核台 -> 照片详情` 中，“相似图片”里的匹配照片ID已改为可点击链接。
- 点击后会进入独立的“相似照片详情”页面，直接查看该相似图对应的站点、工单、检查项、字段，以及该照片的坐标/地址等定位信息。
- 后端新增接口 `GET /api/inspections/photos/{photo_id}/detail`，专门返回独立详情页所需的照片与归属上下文信息。
- **工单审核台字段级审核与历史（2026-04-05）**：
- `web-admin -> 工单审核台 -> 检查项详情` 的字段区域已改为纵向卡片布局，主流程不再依赖左右滑动条；字段值、字段图片、字段审核状态、字段备注集中在同一张卡片内可直接操作。
- 检查项审核支持字段级状态（`pass / warning / fail / pending`）与字段级备注，提交时会同步保存字段审核快照。
- 当“检查项手动结论”与“字段汇总结论”冲突，或仍有未审核字段时，前端会触发双重确认后才允许提交（同一审批人确认）。
- 每个字段支持“历史意见”抽屉，按全部轮次分组展示字段审核历史（轮次倒序、组内时间倒序），并支持多语言备注回显（优先当前语言）。
- 检查项详情新增“检查项审核历史（全部轮次）”入口，支持按轮次查看检查项审核记录，并在每条记录下嵌套展示字段级审核结果与备注（轮次默认展开最新一轮）。
- 该入口已优化到“检查项详情弹窗”标题区右上角（靠近关闭按钮），减少正文区域挤压，视觉层级更清晰。
- 后端新增接口 `GET /api/inspections/detail/{inspection_id}/items/{item_id}/field-reviews/history`，用于字段级历史查询。
- 后端新增接口 `GET /api/inspections/detail/{inspection_id}/items/{item_id}/reviews/history`，用于检查项级历史查询（含嵌套字段级明细）。
- 字段历史接口权限与检查项审核保持一致（`admin / manager / reviewer` 或具备审核写权限），接口本身仅提供只读查询。
- 本轮已补齐该能力相关国际化（中文/英文/印尼语）：字段审核卡片、字段历史抽屉、双重确认弹窗、独立“相似照片详情”页文案均已切换为 i18n 词条。
- 检查项字段审核与字段历史查询接口已增加稳定错误码（如 `CHECK_ITEM_NOT_COMPLETED`、`CHECK_ITEM_COMMENT_REQUIRED`、`REVIEW_PERMISSION_REQUIRED`），前端按错误码做本地化提示，避免多语言界面混入固定中文报错。
- 检查项详情弹窗底部审核按钮已恢复固定显示（通过/警告/不合格）；不再仅在“待审核”状态下显示，是否可操作仍由权限与状态规则控制（不可操作时按钮置灰并展示原因）。
- **工单列表滚轮首滚修复（2026-04-06）**：
- 修复 `web-admin -> 工单列表` 中鼠标滚轮第一下只抖动、第二下才滚动的问题。
- 列表表格区域新增滚轮事件兜底：当鼠标位于表格区域且内部无可滚动子容器时，首次滚轮会直接驱动主内容区滚动。
- 保留原有交互：`Ctrl + 滚轮` 缩放和横向滚动手势不受影响。
- **工单列表新增审核人列（2026-04-06）**：
- `web-admin -> 工单列表` 已新增“审核人”字段，支持在列表里直接看到当前工单审核负责人，无需再进入“审核历史”弹窗查找。
- 展示口径统一为：优先显示工单 `reviewer_id` 对应用户；若历史数据缺失该字段，则回退到检查记录审核人或最近一次 `review_start/final_review` 操作人。
- 空值规则：工单未认领且未审核时，审核人列保持空白（不显示 `-`）。
- **Web 列表页滚轮兜底统一（2026-04-06）**：
- 已在 `web-admin` 全局布局 `MainLayout` 增加表格区域滚轮兜底（`layout-main` 级别），不再只依赖单页面补丁。
- 统一覆盖所有在主内容区展示 `el-table` 的列表页：当表格本身没有可滚动子容器时，首次滚轮会直接滚动主内容区，避免“第一下抖动”。
- 兼容规则：若页面已有局部滚轮逻辑并已 `preventDefault`，全局兜底会自动跳过；`Ctrl + 滚轮` 与横向滚动不受影响。
- **后端脚本清理（2026-03-26）**：
...
- `站点管理 -> LLD Planning Overview` 现在多了一个站点搜索框，可以直接按站点编码或站点名称找记录。
- 这个搜索会和现有的状态、Band、更新时间筛选一起生效，页面不需要再额外展开复杂条件。
- **LLD 下倾角支持负值（2026-04-21）**：
- LLD 的 `机械下倾 / 电子下倾` 现在统一支持负值录入、导入、回显和再次编辑，适用于上倾或反向下倾场景。
- 后端统一校验口径为 `-90 ~ 90`；按 Cell 聚合出来的扇区总下倾也允许负值，不再被旧的 `0 ~ 30` 规则拦截。
- Web 管理端 LLD 编辑弹窗已同步放开负值输入；移动端检查模板中的“下倾角”检查项也已允许负值，避免不同端规则打架。
- **站点地图图例标签修复（2026-03-10）**：
- 已修复站点地图“状态图例”标签取值，统一走国际化状态词条；中英文切换后图例与站点状态胶囊、地图弹窗保持一致。
- **站点详情关键节点时间（2026-03-10）**：
- 站点详情页“站点基本信息”下新增“关键节点时间”模块，现展示安装开始、安装完成、上线、激活、SSV、客户审批通过、PAC 七个节点的状态和时间（无记录显示“未发生”）。
- 现在改为读取统一的“站点生命周期快照”：
- 安装开始=有效开站工单关联下的首次设备绑定（bind/rebind）
- 安装完成=有效开站工单首次 `submitted_at`
- 上线/激活支持两套全局口径，统一在 `系统配置 -> OMC API 配置 -> 站点进度统计口径` 里切换：
- `流程口径（默认）`：上线=有效开站工单首次 `activated_at`，激活=有效开站工单首次 `completed_at`
- `设备事实口径`：上线/激活不依赖工单状态，而是按“开站阶段有效设备集合”全部达到 `ever_online / ever_activated` 时，各设备 `first_online_at / first_activated_at` 的最大值计算；如果站点存在规划设备位，则必须先把规划里的设备位全部完成有效绑定，缺 1 个槽位都不会计入上线/激活
- SSV=有效 SSV 工单首次 `completed_at`
- 客户审批通过 / PAC=仅 Web 端人工上传凭证后确认通过，不参与 `site.status` 主流程推进，只作为独立业务节点和付款条件使用
- 设备更换工单产生的 `maintenance / 回滚状态` 不再污染这些生命周期节点；站点详情、仪表盘概况、趋势图三处口径完全一致。
- 站点详情页现在还会直接显示每个人工节点的上传凭证、操作人、备注，并支持再次上传新凭证覆盖当前结果。
- Web 端“站点详情”和“工单审核台”的设备状态表格现在会优先按站点规划槽位展开：例如规划了 3 个槽位，就默认显示 3 行；未绑定的槽位不会消失，而是直接显示为“未绑定 / 空缺”，方便现场和审核人员一眼看出还差哪个槽位没绑。
- **站点生命周期快照与历史回算（2026-03-15）**：
- 后端新增 `site_progress_snapshots / site_progress_events` 两张表，专门保存站点生命周期读模型和重算/回退记录，不再让 `sites.status` 一张表同时承担“生命周期”和“运行状态”两套语义。
- `site_progress_snapshots` 当前已扩展到客户审批通过、PAC 两个商务节点；它们和安装/上线/激活一样走统一快照读取，但不会反向修改站点运行状态。
- 仪表盘概况、站点事件趋势、站点详情关键节点全部读取快照；如果快照缺失，或快照版本落后于当前代码，读取接口会自动补建/补算。
- 新增管理接口 `POST /api/sites/progress/rebuild`，可对全部站点或指定站点执行全量重算，适合生产环境初始化、历史修正或排障时手动触发。
- Web 管理端已在 `站点管理 -> 站点列表 -> 更多操作` 增加“重算站点生命周期”入口；管理员/站点管理用户可直接在页面上触发一次全量重算，不必再手工调接口。
- 历史趋势按“当前有效事实”回算：如果最后一张有效开站工单后来被作废/删除，对应站点的安装完成、上线、激活会从快照中清掉，历史趋势也会同步减少，不保留失效数据。
...
- **工单列表审核人直显（2026-04-06）**: 工单列表新增“审核人”列。`SUBMITTED` 且未认领时保持空白；认领审核后显示认领人；最终审核后显示最终审核人。历史数据会自动从检查记录或审核日志回填，尽量避免空白。
- **App 端审核人可见性补齐（2026-04-06）**: `uniapp-site-manager` 已补齐审核人展示。工单列表卡片在存在审核人时直接显示“审核人”；检查详情页“审核信息”中，审核人优先读检查详情字段，缺失时回退读取工单详情里的审核人，驳回场景可稳定看到审核人。
- **工单类型**: 开通检查、维护、电源问题、传输问题、GPS问题、信号问题
- **优先级管理**: 低、普通、高、紧急
- **分配和审核**: 支持多级审批和完整审计追踪
- **审核主链统一（2026-03-16）**: Web 管理端审核固定走 `认领审核 -> 检查项审核 -> 最终通过/驳回` 这一条主线；后端 `POST /api/work-orders/{id}/review` 只保留一个最终审核入口，`SUBMITTED` 和 `UNDER_REVIEW` 都能进入终审，Swagger 显示与实际运行保持一致
- **检查项字段问题备注（2026-03-23）**: 在 `web-admin -> 工单审核台 -> 检查项详情` 里，审核人现在可以直接对某个字段填写“问题备注”，只记录真正有问题的字段；提交检查项审核时，系统会自动把这些字段备注汇总进本条检查项的审核意见，审核人仍可在外层补充总体说明，不需要手工重复抄写字段问题。对于历史数据里“只有 `data_value.field_name`、没有模板 `field_id`”的字段行，也允许直接填写问题备注，不再因为旧模板结构缺少 `fields` 而只能只读查看。相关的前端控件文案、检查项审核弹窗标题/按钮、内容列里的“数据/照片/重复图/相似图”标签，以及自动汇总标题都已经补齐到 `中文 / English / Bahasa Indonesia`；检查项审核意见的 `review_comments_i18n` 也会同步生成对应语言版本，字段名会优先使用模板字段的 `label_i18n`，没有时再回退到当前语言下的 legacy 翻译结果。
- **检查表自动补列（2026-03-23）**: 后端启动时会自动检查 `site_inspections / inspection_check_items / inspection_photos` 是否缺少新列，并用 SQLite/MySQL 兼容的方式补齐；像这次新增的 `review_comments_manual / field_issue_comments`，更新代码后只要重启后端就会自动补上，不需要再手动执行迁移文件。
...
- **SSV 创建规则开关（2026-03）**: Web 管理端 `系统配置 -> OMC API 配置` 新增“SSV 创建规则”区块，开关名称为“站点设备激活即可创建SSV”。默认关闭时沿用旧规则，只允许 `site.status = operational` 的站点创建 `SSV`；开启后不再判断站点状态，只要站点设备全部 `ever_activated` 就允许创建 `SSV`，包括 `maintenance` 等非运营中状态。这里的“全部激活”现在也会先检查规划槽位是否已全部有效绑定；如果站点有 3 个规划槽位，只绑了 1 台设备，即便这 1 台已经激活，也不能创建 SSV。
- **站点进度统计口径开关（2026-03-19）**: Web 管理端 `系统配置 -> OMC API 配置` 新增“站点进度统计口径”区块，当前是全局唯一口径。`流程口径` 会把“上线/激活”继续按开站工单 `activated_at / completed_at` 统计；`设备事实口径` 则改为按“开站阶段有效设备集合”全部达到 `ever_online / ever_activated` 的时间统计。无论是哪种口径，只要站点存在规划设备位，系统都会先检查规划槽位是否已经全部完成有效绑定，没绑满就不会把站点算成“已上线 / 已激活”。这个开关会同时影响仪表盘站点概况、站点事件趋势、站点详情关键节点里的“上线 / 激活”。
- **站点付款规则（2026-03-22）**: Web 管理端新增 `系统配置 -> 站点付款规则` 页面，先按“全局一套规则”管理。每条规则都可以自己配：对应节点、是否付款、固定金额或比例、是否依赖“开站工单最终 approve”、warning 是否折减以及折减比例。
- **站点付款记录（2026-03-22）**: 站点详情页新增“付款记录”区块，直接按全局规则实时计算当前站点每个付款节点的状态、应收金额、warning 折减结果和原因说明。比例金额统一按“站点合同金额”计算，所以站点基础信息里也补了 `合同金额` 字段；如果没填合同金额，比例节点会明确提示“待补金额基数”。
- **合同金额批量维护（2026-03-23）**: `站点列表 -> 批量编辑` 现在已经支持直接批量修改 `站点合同金额`；`站点信息维护 -> 批量更新（Excel回写）` 的下载模板、站点列表导出和回写接口也都补上了 `contract_amount` 列，可以走“导出 -> 批量填金额 -> dry-run -> 正式回写”这一套流程。
- **站点付款规则与合同金额文案国际化（2026-03-23）**: `系统配置 -> 站点付款规则` 页面、对应路由标题，以及 `站点列表 -> 批量编辑` / `站点信息维护 -> 批量更新（Excel回写）` 中这次新增的“合同金额”相关文案，已经补齐 `中文 / English / Bahasa Indonesia` 三套词条；切换语言后不再回退成中文硬编码。
- **站点付款币种下拉优化（2026-04-05）**: `系统配置 -> 站点付款规则` 的“币种”已从自由文本改为“可搜索下拉 + 允许自定义输入”。页面内置常用 15 个币种（`USD/CNY/EUR/JPY/IDR/ZAR/NGN/EGP/KES/GHS/TZS/UGX/XOF/XAF/ETB`），也支持直接输入新的币种代码；保存时会自动转成大写。后端接口会返回这组预设选项，旧数据和自定义值保持兼容。
...
- 模板新增/删除分类、检查项、字段后，`draft / in_progress / rejected` 检查会立即同步；`submitted / under_review` 保持当前状态，等下次重新提交时按最新模板校验；`approved / completed / voided` 保持冻结，不再跟着变
- 删除的检查项/字段不会物理清掉历史数据，而是转成隐藏失效；旧照片、旧填写值、旧审核痕迹都会保留
- 第一阶段仍禁止修改高风险结构：检查级别 `level_type`、检查类型 `required_type`、字段类型 `field.type`，以及各类 ID
...
- **提交前最新模板校验（2026-03-15）**:
- Web 提交工单和 UniApp 提交检查前，后端都会先尝试应用该检查当前尚未落地的模板更新，再按最新有效检查项做严格校验
- 如果模板新增了检查项、补充了必填字段、补了字段级必拍照片，已完成状态的旧检查项会自动回到“进行中”，执行端会提示按最新模板补充后再提交
- Web 执行台、UniApp 检查清单/详情页会显示“模板已更新 / 下次重提生效 / 当前已冻结”的提示，避免现场人员看不出模板已经变化
...
- **放弃领货交互加固（移动端，2026-03-21）**: `申请详情` 页不再依赖系统 `showModal(editable)` 输入原因，已改为 App 内自定义原因弹窗，避免不同 Android 机型上出现“输入框不显示、点确认没反应”的兼容问题；失败时会保留已填原因并给出明确错误提示，同时补充业务日志便于按版本/机型排查
- **开始领料权限前置（移动端，2026-03-21）**: `开始领料` 按钮现已前端对齐后端规则，仅申请人可见；仓库侧或其他可查看申请的人不会再看到一个“能点但后端会拒绝”的按钮
...
- **领料辅料默认填充申请数量（移动端，2026-03-30）**: `Start picking -> Picking` 页面进入后会自动按申请数量填充辅料计划值，不再依赖用户手动点击 `Fill max`；若用户已经手动改过某条辅料数量，页面后续刷新/回显不会再把该条重置回默认申请数量
- **领料页印尼语词条补齐（移动端，2026-03-30）**: `Picking` 页已补齐印尼语下缺失的 `issueDraft*` 文案（含提示语、扫描输入提示、提交确认、辅料计划与 `Fill max` 等），避免印尼语界面回退到中文词条
...
### 4. 站点管理
- **站点规划**: 支持站点信息录入和规划管理
- **LLD 下倾角口径（2026-04-21）**: LLD 规划中的 `机械下倾 / 电子下倾` 采用有符号角度，允许负值；系统保存和展示的是原始值，扇区总下倾按两者求和得到，也允许负值。
- **基站状态**: 离线、在线、已激活
- **地理信息**: GPS 坐标管理和位置验证
- **设备关联**: 站点与设备的关联管理
...
- **仪表盘-站点事件趋势（2026-03）**: 仪表盘新增“站点事件趋势”图表，支持 `按日/按周/按月`、`新增量/累计值`、`自动(推荐)/柱状图/折线图` 三组切换。自动模式下：新增量默认柱状图、累计值默认折线图；同时也可手动强制切换图形类型。事件按“站点首次发生时间”统计：`安装开始=仍关联有效开站工单的首次 bind/rebind`、`安装完成=开站工单首次 submitted_at`、`上线/激活=跟随全局“站点进度统计口径”开关读取流程口径或设备事实口径，并且在有站点规划时要求规划槽位先全部有效绑定`、`SSV=SSV工单首次 completed_at`。累计值由“区间前基线 + 各周期新增”计算，避免切换周期后累计曲线从 0 重新起算。趋势分桶时区口径与浏览器本地一致（前端传 `Date.getTimezoneOffset()` 给后端）
...
- **水印距离比对文案国际化（2026-03-14）**: 移动端照片水印中的 `实拍 / 规划 / 距离 / 规划坐标缺失`，以及超距确认弹窗，已统一走 i18n 词条；中文、英文、印尼语会跟随当前语言生成对应文案
