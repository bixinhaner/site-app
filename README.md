...
- `领料单状态 -> Continue picking / 继续领料` 里“无可领物料”这类后端兜底错误也已补齐国际化，不再只在扫码页生效，避免英文或印尼语界面里突然弹中文 toast。
...
npm run build                  # 生产构建
```
- 生产环境不要直接用 `npm run build` 覆盖现有 `dist`。统一在项目根目录执行 `node scripts/publish-web-admin.mjs`：脚本会先完成新构建和资源校验，再原子替换入口，并保留 30 天旧哈希资源。
- **多语言切换（2026-03）**：
...
- **工单审核台字段卡片可读性重做（2026-04-22）**：
- `web-admin -> 工单审核台 -> 检查项详情` 的字段卡片已从“标签 + 弱文本”改为“值优先”的信息层级：字段值使用大字号主视觉显示，单位与照片数量降为次信息，审核结果提升到卡片右上角语义标签。
- 空值、必填未填、缺少说明等风险现在会直接显示成醒目的语义胶囊，不再和普通文本混在一行里；审核人扫一眼就能知道哪些字段需要重点处理。
- AI 建议区也改成了独立信息面板：状态、建议结论、分数、置信度分开展示，分析点和缺失证据分块展示，减少表格式阅读负担。
- 字段区顶部新增汇总胶囊（字段数 / 空值 / 待审 / 警告 / 不合格），方便审核人在进入详情后先看整体风险，再逐项处理。
- 这次改造不依赖后端改接口，仍复用原有字段审核、字段历史、AI 建议和提交流程，只优化前端信息组织与视觉语义。
...
- **物料申请与出入库记录关联（2026-06-13）**：
- `库存管理 -> 出入库记录` 现在会显示来源申请单号；通过物料申请流程产生的出库单、以及基于该出库单产生的退库单，都会保留 `申请单 -> 领料单 -> 出库/退库单` 的追溯关系。
- 出入库记录搜索支持申请单号和领料单号，不再只能靠备注或最近加载的记录查找。
- `库存管理 -> 物料申请 -> 申请单详情` 新增“关联出入库记录”，可直接看到该申请单产生的出库/退库记录，并跳转到领料单详情或出入库历史。
- 移动端同步补齐：`物料申请 -> 申请详情` 会展示关联出入库记录；`退库收货` 的批次列表和收货详情会显示来源申请单、来源领料单，仓库人员不用切到 Web 端才能追溯来源。
- 面向使用者的操作说明见 [物料申请与出入库记录关联使用说明](docs/material-request-stock-trace-user-guide.md)。
- **仪表盘开站与小区扩容口径拆分（2026-06-16）**：
- `仪表盘 -> 开站交付概况` 现在固定表达开站生命周期进度，安装开始、安装完成、部分上线、完全上线、部分激活、完全激活都按开站基线设备位计算，不会因为后续小区扩容把已交付站点从 `3/3` 拉成 `3/6`。
- `仪表盘 -> 小区扩容概况` 新增独立卡片组，仅在系统存在小区扩容工单时展示，统计扩容站点、新增小区/设备、绑定、上线、激活、进行中扩容和已完成扩容。
- 小区扩容新增设备的 OMC 状态会进入扩容概况，不再混入开站交付概况；用户查看交付安装进度和容量扩容进度时不会互相干扰。
- **仪表盘卡片筛选跳转（2026-07-06）**：
- `仪表盘 -> 开站交付概况` 9 个卡片已统一跳转到站点列表，并通过 `site_progress_filter` 按同一仪表盘口径筛出对应站点。支持勘察、规划、安装开始、安装完成、部分上线、完全上线、部分激活、完全激活和 SSV 站点；站点列表会显示“仪表盘筛选”标签，用户可一键清除后回到普通站点列表。设计文档见 [仪表盘开站交付卡片筛选跳转设计](docs/superpowers/specs/2026-07-06-dashboard-site-card-filter-design.md)。
- **Web 首页与通用请求性能优化（2026-08-10）**：
- 仪表盘概况和安装进度分组不再逐站查询规划、开站基线和设备绑定，而是一次批量读取后统一计算；统计口径保持不变，包括“开站基线优先于后续扩容”和“无当前 LLD 时不虚构设备位”。站点数量增加时，数据库查询不会再按站点数线性增长。
- 首页概况、待办、风险和动态改为独立加载，先返回的区块立即展示；工单状态筛选统一使用后端正式参数，后端同时兼容旧客户端的 `status/type` 参数，避免参数被静默忽略后下载全部工单。
- Web 端进入受保护页面前只校验一次登录会话，过期 access token 会先刷新再挂载页面，避免首页并发请求集中返回 401 后重复加载。趋势图表改为按需加载，Vue、Element Plus 和 Axios 拆成稳定依赖包，配合内容哈希缓存降低后续发布的重复下载量。
- 所有后端 HTTP 响应都会带 `Server-Timing` 和 `X-Process-Time-Ms`；超过 `SLOW_REQUEST_THRESHOLD_MS`（默认 500 毫秒）的请求会写入不含参数和请求体的慢请求日志。Caddy 部署模板只对 Vite `/assets/*` 使用一年不可变缓存，SPA 入口仍每次校验，发布后不会卡在旧版本。
- **Web 跨版本菜单加载恢复（2026-08-11）**：
- 用户长时间不刷新页面、期间系统又发布新版本时，点击尚未加载过的菜单不再因旧 JS 哈希文件缺失而表现为“无反应”。客户端会保留目标地址并自动刷新一次；若网络或资源仍异常，会停止循环并显示可操作的重新加载提示，中文、英文和印尼语均已覆盖。
- 生产前端发布统一使用 `node scripts/publish-web-admin.mjs`，按“新哈希资源先就位、入口最后原子替换、旧资源保留 30 天”的顺序执行。这样旧标签页和新标签页可以平滑共存，也不需要为静态页面发布重启 Caddy。
- **后端脚本清理（2026-03-26）**：
...
- **未使用规划项安全删除（2026-08-11）**：施工中或存在进行中开站工单时，LLD 页面允许管理员尝试删除规划 Cell，但后端只接受未绑定 SN、未填写、无照片、无绑定历史且未审核的项目。删除会生成新的 LLD 版本，并同步停用进行中开站工单里的对应检查项；任何已经形成现场事实或处于已提交及以后阶段的工单都会阻止删除，避免规划与 App 工单不一致。
...
- **站点分组与安装进度分组统计（2026-06-02）**：
- Web 管理端新增通用“站点分组”能力。一个维度可以理解成一种看站点的业务口径，例如 `交付范围`、`施工批次`、`客户分组`；每个站点在同一维度下选择一个值，避免统计时重复计数。
- 管理员、经理或规划人员可在 `系统配置 -> 站点分组设置` 自行新增维度、维护选项、设置默认统计维度，也可以停用暂时不用的维度或选项；停用不会删除历史分配，避免影响已维护的站点数据。
- 维度支持两种维护方式：`手工分组` 适合施工批次、交付包这类人为口径；`按字段自动分组` 允许用户自己命名维度，再从白名单字段中选择来源字段，例如站点基础字段（城市、省份、站点类型、状态等）或当前 LLD 小区字段（Duplex Mode、Band、RAT、Cluster 等）。字段派生可按字段值自动生成选项，也可用关键词规则归到已有选项；预览只做试算，不保存配置也不写入分组，同步时才保存当前配置并写入站点分组。同一站点匹配到多个选项时保留人工确认，不会自动覆盖。
- `仪表盘 -> 安装进度分组` 会按当前分组维度展示总站点、已开始、安装完成、未安装、上线、激活和 SSV 完成数量。点击“查看站点”会跳到站点列表，并自动带上同一分组筛选；需要调整分组规则时从这里进入“分组设置”，仪表盘本身不再执行写入动作。
- `仪表盘 -> 安装进度分组` 已补齐中英文印尼语文案：标题、副标题、分组维度选择、表头、按钮、空状态、错误提示、设备上线/激活明细，以及系统内置 `交付范围 / 未分组` 显示名都会跟随当前语言切换。
- Savanna 项目的 `交付范围` 可在 `系统配置 -> 站点分组设置` 中按当前 LLD 的 `Duplex Mode` 预览或同步：只含 TDD 的站点归入 `TDD`，只含 FDD 的站点归入 `FDD`，同时包含 TDD/FDD 或无法识别的站点保留人工确认。预览/同步结果会优先显示覆盖、冲突、跳过、写入这些需要关注的行，最多返回 500 条样例，避免关键异常被普通不变记录淹没。
- 历史环境如果已经存在编码为 `delivery_scope` 的“交付范围”维度，系统启动时会补齐它的 LLD Duplex 派生配置；这样升级后在设置页能直接看到来源字段和预览/同步入口。
- `站点列表` 支持按分组维度和分组选项筛选，也会直接显示站点分组；批量编辑弹窗、站点详情编辑弹窗、站点列表导出和 Excel 回写都会带上 `group:维度名` 列，方便批量维护。
- 站点接口会把站点基础信息和分组信息分开返回，避免生成分组后影响站点列表、站点地图和分组筛选加载。
- **登录后首页兜底（2026-03）**：
...
- **工单列表导出（2026-05-11）**: `web-admin -> 工单管理 -> 工单列表` 现在支持直接导出“当前筛选结果”到 Excel，不再需要手工复制表格。导出会跟随页面当前语言输出文件名、Sheet 名、表头，以及工单类型/状态/优先级/风险字段文案；当筛选结果过大时，后端会拦截并提示先缩小范围，避免一次性导出把页面拖慢。
- **小区扩容工单（2026-05-28）**: 已安装或已开通站点从 `3 扇区 / 3 小区` 扩到 `3 扇区 / 6 小区` 时，不再直接覆盖当前 LLD。Web 管理端新建 `小区扩容` 工单时，创建面板只显示目标 LLD 准备状态，用户在独立弹窗里完成 `下载底稿 -> 上传完整 LLD -> 差异预览 -> 使用此目标 LLD`。所有新建工单都必须选择检查模板；创建面板直接展示全部可用模板，不再依赖模板绑定或 `task_type` 筛选，避免生产库未配置绑定时下拉为空。小区扩容仍会优先默认选中该站点最近一次有效开站工单实际使用过的模板；没有历史模板时由用户从完整模板列表中选择。小区扩容只允许 `pending_online / online_pending_activation / operational / maintenance` 这些安装后的站点状态，Web 新建时站点下拉会提前过滤，后端创建和 LLD 预览继续兜底校验；`construction` 或更早阶段的站点应走开站工单或直接维护目标 LLD。差异识别保持现有 LLD 数据结构不变，按 `local_cell_id + band` 识别新增小区/设备，同时按方位角和现网扇区聚合物理扇区，避免把 LCID 11/12/13 误算成新增物理扇区。执行人只处理新增小区/设备相关检查项和 SN 绑定；审核通过后，OMC 只检查这些新增目标，全部曾上线后进入“已上线待激活”，全部曾激活后才完成工单并把目标 LLD 合并成新的当前规划。未安装站点仍走原来的 LLD 导入/编辑流程，不需要创建扩容工单。新增类型、目标 LLD 弹窗、审核页扩容信息，以及 Web/App 端可见的扩容类型文案已补齐 `中文 / English / Bahasa Indonesia`。用户操作步骤见 `docs/cell-expansion-user-guide.md`。
...
- **工单类型**: 开通检查、小区扩容、设备更换、维护、电源问题、传输问题、GPS问题、信号问题
- **优先级管理**: 低、普通、高、紧急
- **分配和审核**: 支持多级审批和完整审计追踪
...
- **作废阻断提示（2026-08-11）**: 工单因设备未解绑而无法作废时，Web 会用阻断弹窗列出具体 `SN / 扇区 / Band`，并明确提示由工单执行人在 App 对应工单中先解绑；后端保护规则不变，避免管理员误以为系统故障或绕过设备归属校验
...
- **仪表盘部分/完全上线激活统计（2026-06-04）**: `Dashboard -> 站点概况` 中“上线站点 / 激活站点”已拆成“部分上线站点、完全上线站点、部分激活站点、完全激活站点”。“完全”继续沿用站点进度快照口径；“部分”按开站阶段有效设备位统计，只要不是完全站点且有一部分设备达到 `ever_online / ever_activated` 就会计入。四张卡片下方会用小字显示设备分数，例如 `设备上线 2/3`，分母优先使用规划设备位数量，没规划时使用当前有效绑定设备位数量。下方“站点进度分组”表也使用同一口径，按业务分组展示部分/完全上线、部分/完全激活，并在每个分组单元格中显示上线设备或激活设备分数。
- **OMC 请求保护与设备快照（2026-06-04）**: Web 管理端 `系统配置 -> OMC API 配置` 包含“请求保护”“设备快照”和“运行状态”。后端会缓存 OMC access token，不再每查一个 SN 都重新拿 token；后台轮询、手动刷新、单 SN 查询和 cellName 同步统一走全局限速器，默认 `120 次/分钟`、短时突发 `10`。后台轮询会先调用 `/northboundApi/v1/device/group` 自动发现当前账号可访问的设备分组；若配置里未手动选择分组，就只查询自动发现到的叶子分组，父级分组会跳过，避免 OMC 对父级组固定返回 `402 no device group operation permission`；若手动选择了分组，则只查询所选分组。每个分组再按配置调用 `/northboundApi/v1/device/query` 拉取设备快照（默认周期 `300` 秒、每页 `1000`），最终按 SN 去重后写入聚合表。只有当快照实际覆盖到设备 SN 时，后台才用快照覆盖范围过滤后续 `/enodeb/infos/status/{sn}`；如果快照接口返回成功但 `rows=0/covered=0`，或快照拉取失败，系统会自动回退到受全局限速器保护的逐 SN 查询，避免 OMC 库存权限异常时把待确认工单长期卡住。快照里的 `connection_status=On` 会推进 `ever_online`；`connection_status=Off` 且 `offlineDays` 有值时，也会作为“曾上线”证据补漏并推算一个上线证据时间。该快照接口不能证明“曾激活”，激活仍以 SN 状态接口或手工确认等可靠来源为准。后台每轮 OMC 检查会把设备检查摘要可靠写回工单 `extra_data.omc_status`，因此工单状态推进后，前端详情也能看到本轮检查过哪些 SN、缺哪些上线或激活。运行状态会显示最近 1/5/15 分钟请求量、失败数、等待请求、轮询队列和最近请求明细；最近请求支持打开详情抽屉查看脱敏后的 request/response body，便于直接排查 `device/query` 空结果、OMC 业务 `402` 等问题。请求详情只保留在后端进程内存中，不写入数据库；password、token 等敏感字段会脱敏，大响应会截断。
- **站点付款规则（2026-03-22，2026-07-02 更新）**: Web 管理端 `系统配置 -> 站点付款规则` 支持“默认付款方案 + 分包商付款方案”。默认方案兜底所有站点；当站点通过“站点分组”分配了系统配置中选定的分包商分类，且该分包商配置了独立方案时，系统会自动套用对应付款节点、单价和比例。每条规则都可以自己配：对应节点、是否付款、固定金额或比例、是否依赖“开站工单最终 approve”、warning 是否折减以及折减比例。
- **站点付款记录（2026-03-22，2026-07-02 更新）**: 站点详情页“付款记录”区块会直接展示当前命中的付款方案、分包商和金额基数。比例金额可按站点合同金额计算，也可按分包商方案里的站点单价计算；如果缺少对应金额基数，比例节点会明确提示“待补金额基数”。
- **账号分包商与工单指派联动（2026-07-02）**: 用户管理里可以给账号绑定一个分包商；`系统配置 -> Web工单执行配置 -> 工单指派联动` 中先选择哪个“站点分组分类”代表分包商，再开启“自动同步站点分包商”。创建或重新指派工单时，如果执行账号绑定了分包商且站点还没有分包商，系统会自动把站点关联到该分包商。若站点已有不同分包商，系统会阻止保存并提示人工确认，避免静默改错付款口径。分类名称可以由用户自定义，不使用分包商的项目无需配置，默认不影响原有工单指派流程。
...
- 模板同步只更新检查项结构和字段配置，不会覆盖已绑定的设备 `SN`；设备绑定/解绑必须继续走绑定接口，确保当前检查项、绑定历史和设备状态一致
...
- **库存台账重设计（Web，2026-08-10）**: `/inventory/list` 按主设备和辅材分别展示。主设备以 SN 为唯一口径，直接拆分在库、已出库、待检查、已检查、退库待收和异常状态；辅材按仓库数量展示当前库存、已出库未归还、补货点和库存状态，避免把不同单位的辅材强行汇总成一个总数。页面支持按设备/仓库切换、URL 筛选、筛选结果导出和逐层下钻，所有汇总数字都能追到 SN、仓库或出入库单据；类别标签位于统计概览之前，概览采用紧凑布局，导出和刷新与结果数量合并显示，让首屏优先呈现更多库存行；辅材零库存通过“零库存项”统计、显示开关、在库数量和状态统一表达，不再额外显示容易误解的仓库/辅材计数列；树形表的展开箭头与名称对齐，父级名称也可通过鼠标或键盘展开；中文库存单位会按当前中文、英文或印尼语转换显示，但不改动数据库原值。
- **补货点语义统一（2026-08-10）**: 辅材 `min_stock=0` 统一表示“未配置补货点”，不会再被计入低库存；表格会如实标记该状态，但不把这项次要配置提示放在概览卡片或库存状态筛选中。点击零库存或需补货卡片时，会直接显示对应记录，不受默认隐藏零库存开关影响。
...
- **废弃仓库删除（2026-05-15）**: `web-admin -> 仓库管理` 支持删除废弃仓库；系统实际执行“停用”而不是物理删库，删除后该仓库不会再出现在入库、出库、申请、退库选仓列表，也不再计入仓库负责人的管理范围。已有历史出入库记录保留原仓库名称，方便追溯；若仓库仍有库存、当前设备实例、未完成物料申请、未完成领料单或待处理退库单，系统会阻止删除并提示先处理这些事项。
...
### OMC 管理
- `GET /api/omc/config` - 获取 OMC API 配置，包含基础地址、超时时间、手工确认开关、请求限速、突发上限、token 缓存时间和设备快照配置
- `PUT /api/omc/config` - 更新 OMC API 配置；`rate_limit_per_minute` 默认 120，`rate_limit_burst` 默认 10，`token_ttl_seconds` 默认 600；设备快照默认开启，`inventory_snapshot_interval_seconds=300`，`inventory_device_group_ids=[]` 表示自动发现并查询全部可访问分组，`inventory_page_size=1000`，`offline_days_marks_ever_online=true`
- `GET /api/omc/device-groups` - 从 OMC 实时读取当前 API 账号可访问的设备分组，用于在配置页手动限定快照同步范围
- `GET /api/omc/runtime` - 查看当前后端进程内的 OMC 出口请求运行状态，包括最近请求量、失败数、限速等待、token 缓存命中和后台轮询队列
- `GET /api/omc/states` - 查询 SN 维度 OMC 状态聚合表
- `GET /api/omc/devices/{sn}/status` - 立即查询单个 SN 的 OMC 状态；该请求也会进入全局限速器。若 OMC 返回业务失败码（如 `401/402`），接口会返回错误 payload 供排查，但不会把该 SN 的原始在线/激活状态误写成 `false`

### 库存管理
- `GET /api/stock` - 库存查询
- `GET /api/stock/inventory/overview` - 新库存页聚合数据（主设备/辅材、按设备/仓库、筛选与分页）
- `GET /api/stock/inventory/main-instances` - 主设备 SN 生命周期、领用人和站点扇区下钻明细
- `GET /api/stock/inventory/auxiliary-details/{equipment_id}` - 辅材仓库分布、已出库未归还和出入库流水明细
- `DELETE /api/stock/warehouses/{id}` - 删除废弃仓库（实际停用；有库存或未完成单据时会阻止）
...
- **站点坐标纠偏后的照片距离刷新（2026-05-19）**: 当站点经纬度被修正时，后端会自动重算该站点历史检查照片的“距规划距离”和超阈值结论；Web 审核端会显示“已按当前站点坐标刷新”，避免旧照片继续沿用坐标填反时留下的错误距离。
