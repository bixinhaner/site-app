...
- `PUT /api/omc/config` - 更新 OMC API 配置；`rate_limit_per_minute` 默认 120，`rate_limit_burst` 默认 10，`token_ttl_seconds` 默认 600；设备快照默认开启，`inventory_snapshot_interval_seconds=300`，`inventory_device_group_ids=[30]`，`inventory_page_size=1000`，`offline_days_marks_ever_online=true`
