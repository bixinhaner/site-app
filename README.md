...
- `设备事实口径`：上线/激活不依赖工单状态，而是按“开站阶段有效设备集合”全部达到 `ever_online / ever_activated` 时，各设备 `first_online_at / first_activated_at` 的最大值计算
...
- **站点进度统计口径开关（2026-03-19）**: Web 管理端 `系统配置 -> OMC API 配置` 新增“站点进度统计口径”区块，当前是全局唯一口径。`流程口径` 会把“上线/激活”继续按开站工单 `activated_at / completed_at` 统计；`设备事实口径` 则改为按“开站阶段有效设备集合”全部达到 `ever_online / ever_activated` 的时间统计。这个开关会同时影响仪表盘站点概况、站点事件趋势、站点详情关键节点里的“上线 / 激活”。
