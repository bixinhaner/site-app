...
- **扇区扩容工单（2026-05-28）**: 已安装或已开通站点从 3 扇区扩到 6 扇区时，不再直接覆盖当前 LLD。Web 管理端新建 `扇区扩容` 工单时先上传目标 LLD 做预览，系统只识别新增设备位，并拒绝删除或改写已有 Cell 的关键字段；执行人只处理新增槽位的检查和设备绑定。审核通过后，OMC 只检查这些新增设备位，全部曾上线后进入“已上线待激活”，全部曾激活后才完成工单并把目标 LLD 合并成新的当前规划。未安装站点仍走原来的 LLD 导入/编辑流程，不需要创建扩容工单。
