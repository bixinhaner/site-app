...
- **小区扩容工单（2026-05-28）**: 已安装或已开通站点从 `3 扇区 / 3 小区` 扩到 `3 扇区 / 6 小区` 时，不再直接覆盖当前 LLD。Web 管理端新建 `小区扩容` 工单时，创建面板只显示目标 LLD 准备状态，用户在独立弹窗里完成 `下载底稿 -> 上传完整 LLD -> 差异预览 -> 使用此目标 LLD`。系统必须选择开站检查模板，并优先默认选中该站点最近一次有效开站工单实际使用过的模板；没有历史模板时再按当前模板绑定规则推荐；生产库缺少模板绑定记录时会回退展示已有检查模板，避免下拉为空。小区扩容只允许 `pending_online / online_pending_activation / operational / maintenance` 这些安装后的站点状态，Web 新建时站点下拉会提前过滤，后端创建和 LLD 预览继续兜底校验；`construction` 或更早阶段的站点应走开站工单或直接维护目标 LLD。差异识别保持现有 LLD 数据结构不变，按 `local_cell_id + band` 识别新增小区/设备，同时按方位角和现网扇区聚合物理扇区，避免把 LCID 11/12/13 误算成新增物理扇区。执行人只处理新增小区/设备相关检查项和 SN 绑定；审核通过后，OMC 只检查这些新增目标，全部曾上线后进入“已上线待激活”，全部曾激活后才完成工单并把目标 LLD 合并成新的当前规划。未安装站点仍走原来的 LLD 导入/编辑流程，不需要创建扩容工单。新增类型、目标 LLD 弹窗、审核页扩容信息，以及 Web/App 端可见的扩容类型文案已补齐 `中文 / English / Bahasa Indonesia`。用户操作步骤见 `docs/cell-expansion-user-guide.md`。
