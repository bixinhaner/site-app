...
- `仪表盘 -> 安装进度分组` 会按当前分组维度展示总站点、已开始、安装完成、未安装、上线、激活和完成率。点击“查看站点”会跳到站点列表，并自动带上同一分组筛选。
- Savanna 项目可在仪表盘点击“生成交付范围”，系统会先根据当前 LLD 的 `Duplex Mode` 做试运行预览：只含 TDD 的站点归入 `TDD`，只含 FDD 的站点归入 `FDD`，同时包含 TDD/FDD 或无法识别的站点保留人工确认。确认后才写入分组。
