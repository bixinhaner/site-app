...
- 库存链路里还有一层“仓库可管理范围”：像 `退库收货 / 待确认出库` 这类仓库侧页面，给了 `inventory:return:write` 或 `inventory:issue-draft:write` 只代表“允许操作该功能”；实际能看到哪些单据，还受“当前账号是否绑定为仓库负责人”限制。只有 `admin` 或显式授予 `inventory:warehouse:write` 的角色，才默认视为可看全部仓库。
