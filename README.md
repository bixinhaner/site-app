...
- **OMC 请求保护（2026-06-03）**: Web 管理端 `系统配置 -> OMC API 配置` 新增“请求保护”和“运行状态”。后端现在会缓存 OMC access token，不再每查一个 SN 都重新拿 token；后台轮询、手动刷新、单 SN 查询和 cellName 同步统一走全局限速器，默认 `120 次/分钟`、短时突发 `10`。后台 OMC 轮询也改成按工单逐个处理、逐个提交，避免几百次请求集中在一分钟内打到 OMC。运行状态会显示最近 1/5/15 分钟请求量、失败数、等待请求、轮询队列和最近请求明细，排查 OMC 黑名单或限速问题时不再只能看日志猜。
...
### OMC 管理
- `GET /api/omc/config` - 获取 OMC API 配置，包含基础地址、超时时间、手工确认开关、请求限速、突发上限和 token 缓存时间
- `PUT /api/omc/config` - 更新 OMC API 配置；`rate_limit_per_minute` 默认 120，`rate_limit_burst` 默认 10，`token_ttl_seconds` 默认 600
- `GET /api/omc/runtime` - 查看当前后端进程内的 OMC 出口请求运行状态，包括最近请求量、失败数、限速等待、token 缓存命中和后台轮询队列
- `GET /api/omc/states` - 查询 SN 维度 OMC 状态聚合表
- `GET /api/omc/devices/{sn}/status` - 立即查询单个 SN 的 OMC 状态；该请求也会进入全局限速器
