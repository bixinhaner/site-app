...
- **仪表盘部分/完全上线激活统计（2026-06-04）**: `Dashboard -> 站点概况` 中“上线站点 / 激活站点”已拆成“部分上线站点、完全上线站点、部分激活站点、完全激活站点”。“完全”继续沿用站点进度快照口径；“部分”按开站阶段有效设备位统计，只要不是完全站点且有一部分设备达到 `ever_online / ever_activated` 就会计入。四张卡片下方会用小字显示设备分数，例如 `设备上线 2/3`，分母优先使用规划设备位数量，没规划时使用当前有效绑定设备位数量。
