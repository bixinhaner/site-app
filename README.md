...
- **OMC 请求保护与设备快照（2026-06-04）**: Web 管理端 `系统配置 -> OMC API 配置` 包含“请求保护”“设备快照”和“运行状态”。后端会缓存 OMC access token，不再每查一个 SN 都重新拿 token；后台轮询、手动刷新、单 SN 查询和 cellName 同步统一走全局限速器，默认 `120 次/分钟`、短时突发 `10`。后台轮询会先调用 `/northboundApi/v1/device/group` 自动发现当前账号可访问的设备分组；若配置里未手动选择分组，就查询所有发现到的分组，若手动选择了分组，则只查询所选分组。每个分组再按配置调用 `/northboundApi/v1/device/query` 拉取设备快照（默认周期 `300` 秒、每页 `1000`），最终按 SN 去重后写入聚合表，再只对快照覆盖到的 SN 继续查 `/enodeb/infos/status/{sn}`，避免未知或无权限 SN 反复触发 OMC `401/402`。快照里的 `connection_status=On` 会推进 `ever_online`；`connection_status=Off` 且 `offlineDays` 有值时，也会作为“曾上线”证据补漏并推算一个上线证据时间。该快照接口不能证明“曾激活”，激活仍以 SN 状态接口或手工确认等可靠来源为准。运行状态会显示最近 1/5/15 分钟请求量、失败数、等待请求、轮询队列和最近请求明细，排查 OMC 黑名单或限速问题时不再只能看日志猜。
