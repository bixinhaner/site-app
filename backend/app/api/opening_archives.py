...
from app.models.opening_archive import SiteOpeningArchive, SiteOpeningArchiveVersion, SiteOpeningArchiveKVIndex
from app.models.inspection import SiteInspection
from app.services.opening_archive_service import (
...
def page_archives(
    ...
    q = (
        db.query(SiteOpeningArchive)
        .options(
            joinedload(SiteOpeningArchive.site),
            joinedload(SiteOpeningArchive.inspection).joinedload(SiteInspection.inspector),
            joinedload(SiteOpeningArchive.inspection).joinedload(SiteInspection.reviewer),
        )
        .join(Site, Site.id == SiteOpeningArchive.site_id)
    )
    if site_id:
        q = q.filter(SiteOpeningArchive.site_id == site_id)
    if template_id:
        q = q.filter(SiteOpeningArchive.template_id == template_id)
    if keyword:
        kw = f"%{keyword}%"
        q = q.filter((Site.site_name.like(kw)) | (Site.site_code.like(kw)))
    if date_from:
        q = q.filter(SiteOpeningArchive.updated_at >= date_from)
    if date_to:
        q = q.filter(SiteOpeningArchive.updated_at <= date_to)

    total = q.count()
    rows = (
        q.order_by(SiteOpeningArchive.updated_at.desc())
...
def get_archive(
    ...
    a = db.query(SiteOpeningArchive).filter(SiteOpeningArchive.id == archive_id).first()
    if not a:
...
def list_history(
    ...
    vers = db.query(SiteOpeningArchiveVersion).filter(
        SiteOpeningArchiveVersion.archive_id == archive_id
    ).order_by(SiteOpeningArchiveVersion.version.asc()).all()
...
async def export_archive_zip(
    ...
    a = db.query(SiteOpeningArchive).options(
        joinedload(SiteOpeningArchive.site),
        joinedload(SiteOpeningArchive.inspection).joinedload(SiteInspection.inspector),
        joinedload(SiteOpeningArchive.inspection).joinedload(SiteInspection.reviewer),
...
async def export_archive_pdf(
    ...
    for cat in cats:
        ...
        for it in (cat.get("items") or []):
            ...
            if sectors:
                ...
                for sec in sectors:
                    ...
                    for k, v in vals.items():
                        sec_rows.append([
                            sec.get("sector_id") or "-",
                            next((fd.get("label") for fd in fields if fd.get("field_id")==k), k),
                            fmt_value_by_field(next((fd for fd in fields if fd.get("field_id")==k), {}), v)
                        ])
                if len(sec_rows) > 1:
                    t = Table(sec_rows, colWidths=[2.5*cm, 4*cm, 7*cm])
                    t.setStyle(TableStyle([
                        ('FONTNAME', (0,0), (-1,-1), FONT_MAIN),
                        ('FONTSIZE', (0,0), (-1,-1), 10),
                        ('BACKGROUND', (0,0), (-1,0), primary),
            ...
            if cells:
                cell_rows = [["小区", "扇区", "Band", "字段", "值"]]
                for cell in cells:
                    vals = (cell.get("values") or {})
                    for k, v in vals.items():
                        cell_rows.append([
                            cell.get("cell_id") or "-",
                            cell.get("sector_id") or "-",
                            cell.get("band") or "-",
                            next((fd.get("label") for fd in fields if fd.get("field_id")==k), k),
                            fmt_value_by_field(next((fd for fd in fields if fd.get("field_id")==k), {}), v)
                        ])
                if len(cell_rows) > 1:
                    t = Table(cell_rows, colWidths=[2.2*cm, 2.0*cm, 2.0*cm, 3.5*cm, 5.3*cm])
                    t.setStyle(TableStyle([
                        ('FONTNAME', (0,0), (-1,-1), FONT_MAIN),
                        ('FONTSIZE', (0,0), (-1,-1), 10),
                        ('BACKGROUND', (0,0), (-1,0), primary),
...
def rebuild_index(
    ...
    v = db.query(SiteOpeningArchiveVersion).filter(
        SiteOpeningArchiveVersion.archive_id == archive_id,
        SiteOpeningArchiveVersion.version == ver,
    ).first()
...
def search_kv(
    ...
    q = db.query(SiteOpeningArchiveKVIndex).join(
        SiteOpeningArchive, SiteOpeningArchive.id == SiteOpeningArchiveKVIndex.archive_id
    )
    if path_prefix:
        like = f"{path_prefix}%"
        q = q.filter(SiteOpeningArchiveKVIndex.path.like(like))
    if keyword:
        kw = f"%{keyword}%"
        q = q.filter((SiteOpeningArchiveKVIndex.value_string.like(kw)) | (SiteOpeningArchiveKVIndex.raw_json.isnot(None)))
    if site_id:
        q = q.filter(SiteOpeningArchive.site_id == site_id)
    rows = q.order_by(SiteOpeningArchiveKVIndex.updated_at.desc()).limit(limit).all()
    return [
        {
...
def rebuild_for_work_order(
    ...
    """为指定开站工单（需已产生 inspection_id）创建/追加档案。"""
    _require_editor(current_user)
    ...
    if wo.type not in [WorkOrderTypeEnum.OPENING_INSPECTION]:
        raise HTTPException(status_code=400, detail="仅支持开站工单重建档案")
