...
async def create_work_order(
    ...
    if data.type == WorkOrderTypeEnum.CELL_EXPANSION and normalized_expansion:
        ...
        extra_data["expansion_summary"] = {
            "current_planning_id": normalized_expansion.get("current_planning_id"),
...
async def accept_work_order(
    ...
    elif template_data and _template_requires_lld_cells(template_data):
        target_cells = (wo.extra_data or {}).get("expansion_target_cells") or []
        if not target_cells:
            raise HTTPException(
                status_code=400,
...
async def manual_confirm_opening_omc_status(
    ...
    if wo.type == WorkOrderTypeEnum.CELL_EXPANSION and wo.inspection_id:
        bound_sns = sorted({
            ...
            .all()
            if str(item.equipment_sn or "").strip()
        })
    else:
