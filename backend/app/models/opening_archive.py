...
class SiteOpeningArchive(Base):
    """开站档案（当前版本快照）"""
    __tablename__ = "site_opening_archives"
    ...
    versions = relationship("SiteOpeningArchiveVersion", back_populates="archive", cascade="all, delete-orphan")


class SiteOpeningArchiveVersion(Base):
    """开站档案版本历史"""
    __tablename__ = "site_opening_archive_versions"

    id = Column(String(32), primary_key=True)
    archive_id = Column(String(32), ForeignKey("site_opening_archives.id"), index=True, nullable=False)
    ...
    archive = relationship("SiteOpeningArchive", back_populates="versions")
    changer = relationship("User", foreign_keys=[changed_by])


class SiteOpeningArchiveKVIndex(Base):
    """开站档案 KV 索引"""
    __tablename__ = "site_opening_archive_kv_index"
