<template>
	<view class="page" :key="languageStore.currentLocale">
		<CustomNavbar :title="$t('stock.returnReceivingEntry')" :showBack="true" variant="brand" />
		...
		>
			...
			<view class="list">
				...
				<EmptyState
					...
					:description="$t('stock.returnReceivingListEmptyTip')"
				/>
					...
					<view v-for="batch in records" :key="batch.batch_id" class="card u-card">
						<view class="card-head">
							<view class="left">
								...
								<view class="meta">
									<text class="meta-item">{{ $t('stock.returnBatchCreatedTime') }}: {{ timeAgo(batch.created_at) }}</text>
									<text class="dot">·</text>
									<text class="meta-item">{{ $t('stock.returnBatchLatestTime') }}: {{ timeAgo(batch.latest_created_at) }}</text>
								</view>
							</view>
							<view class="right">
						...
						<view v-if="batch.documents.length > 0" class="doc-box">
							...
							<view v-for="doc in visibleDocs(batch)" :key="doc.id" class="doc-item">
								...
								<view class="doc-meta">
									...
									<text class="dot">·</text>
									<text class="doc-meta-item">{{ doc.operator_name || '-' }}</text>
								</view>
								<view class="doc-meta">
								...
								<view class="doc-actions">
									<button class="u-btn u-btn-primary u-btn-xs u-pressable" @click="goDetail(doc)">
										{{ canHandleDoc(doc) ? $t('stock.returnReceivingAction') : $t('stock.issueConfirmActionView') }}
									</button>
									<text
										v-if="String(doc.status || '') === 'rejected'"
										class="doc-reject-text u-pressable"
										@click="openDocRejectReason(doc)"
									>
										{{ $t('stock.returnBatchViewRejectReason') }}
									</text>
								</view>
							</view>
...
<script setup>
	...
	const ensureAccess = () => guardRouteAccess({
		userStore,
		route: 'pages/stock/returns/receive-list',
		t: $t,
	...
	const toggleDocs = (batch) => {
		const key = String(batch?.batch_id || '').trim()
		if (!key) return
		expandedDocsMap.value = { ...(expandedDocsMap.value || {}), [key]: !isDocsExpanded(batch) }
	}
	...
	const collectRejectRows = (batch) => {
		...
		for (const doc of batch?.documents || []) {
			...
			rows.push({
				document_number: String(doc?.document_number || '-'),
				reason: String(doc?.approval_comments || '').trim() || $t('stock.returnBatchRejectReasonEmpty'),
			})
		}
		if (rows.length > 0) return rows
		return (batch?.reject_reasons || []).map(reason => ({
			document_number: '-',
	...
	const canHandleDoc = (doc) => ['pending_receive', 'partially_received'].includes(String(doc?.status || ''))
	...
	const load = async (reset = true) => {
		...
		try {
			...
			const url = `${buildApiUrl(API_ENDPOINTS.STOCK.RETURN_WORKBENCH_BATCHES)}?${params.join('&')}`
	...
	const goDetail = (doc) => {
		...
		if (!id) return
		uni.navigateTo({ url: `/pages/stock/returns/receive?return_id=${id}` })
	}
