export default {
  app: {
    systemName: "Sistem Manajemen Informasi Situs",
    systemSubtitle:
      "Platform Manajemen Peralatan Stasiun Pangkalan Telekomunikasi",
    pageSuffix: "Sistem Manajemen Informasi Situs",
  },
  common: {
    home: "Beranda",
    language: "Bahasa",
    chinese: "Bahasa Mandarin",
    english: "Bahasa Inggris",
    indonesian: "Bahasa Indonesia",
    cancel: "Membatalkan",
    logout: "Keluar",
    logoutSuccess: "Berhasil keluar",
    logoutFailed: "Gagal keluar",
    userFallback: "Pengguna",
  },
  login: {
    username: "Nama pengguna",
    password: "Kata sandi",
    submit: "Masuk",
    submitting: "Masuk...",
    success: "Masuk berhasil!",
    failed: "Gagal masuk",
    failedRetry: "Gagal masuk, coba lagi nanti",
    invalidCredentials: "Nama pengguna atau kata sandi tidak valid",
    accountDisabled:
      "Akun ini telah dinonaktifkan. Silakan hubungi administrator.",
    webLoginForbidden: "Akun ini tidak diperbolehkan masuk ke admin web.",
    appLoginForbidden:
      "Akun ini tidak diizinkan untuk masuk ke aplikasi seluler.",
    usernameRequired: "Silakan masukkan nama pengguna",
    usernameLength: "Panjang nama pengguna harus 2 hingga 50 karakter",
    passwordRequired: "Silakan masukkan kata sandi",
    passwordLength: "Kata sandi minimal harus 6 karakter",
  },
  inventory: {
    groups: {
      flow: "Aliran Masuk/Keluar",
      asset: "Pengaturan Aset",
      ledger: "Buku besar",
      other: "Lainnya",
    },
    page: {
      summary: "Ringkasan inventaris",
      mainDeviceInventory: "Inventaris Perangkat Utama",
      auxiliaryInventory: "Inventaris Material Pendukung",
      byEquipment: "Menurut Peralatan",
      byWarehouse: "Menurut Gudang",
      filters: "Filter inventaris",
      searchMain: "Cari SN, nama, atau kode peralatan",
      searchAuxiliary: "Cari nama atau kode material",
      searchSnSite: "Cari SN atau situs",
      searchWarehouse: "Cari gudang",
      searchDocumentOwner: "Cari dokumen atau penerima",
      warehouse: "Gudang",
      warehouses: "Gudang",
      allWarehouses: "Semua gudang",
      unassignedWarehouse: "Gudang Belum Ditentukan",
      warehouseWithUnassigned: "{count} gudang · {unassigned} belum ditentukan",
      deviceStatus: "Status perangkat",
      units: {
        device: "unit",
        set: "set",
        piece: "buah",
        pair: "pasang",
        meter: "m",
        rod: "batang",
        strip: "buah",
        roll: "gulung",
        box: "kotak",
        batch: "batch",
      },
      showZeroStock: "Tampilkan stok nol",
      exportExcel: "Ekspor Excel",
      exportSuccess: "Berhasil mengekspor {count} data",
      exportFailed: "Ekspor gagal",
      refresh: "Segarkan",
      retry: "Muat ulang",
      close: "Tutup",
      clearFilters: "Hapus filter",
      noMatchingInventory: "Tidak ada peralatan yang cocok",
      adjustFilters: "Ubah kata pencarian atau filter",
      noInventoryRecords: "Belum ada catatan inventaris",
      inventoryEmptyHint: "Data inventaris akan tampil di sini",
      noDeviceInstances: "Belum ada instans perangkat",
      noOutboundPending: "Tidak ada barang keluar yang belum kembali",
      noTransactions: "Belum ada transaksi stok",
      loadFailed: "Gagal memuat inventaris",
      equipmentCount: "{count} jenis peralatan",
      auxiliaryCount: "{count} jenis material",
      warehouseCount: "{count} gudang",
      inventoryRecordCount: "{count} catatan inventaris",
      hiddenZeroRecords: "{count} catatan stok nol disembunyikan",
      totalRows: "Total {count} data",
      auxiliaryTypes: "Jenis Material",
      inventoryRecords: "Catatan Inventaris",
      stockedRecords: "Catatan Berstok",
      zeroRecords: "Item Stok Nol",
      needsRestock: "Perlu Restok",
      equipmentCodeName: "Kode / Nama Peralatan",
      equipmentCode: "Kode Peralatan",
      equipmentName: "Nama Peralatan",
      warehouseDistribution: "Distribusi Gudang",
      equipmentTypes: "Jenis Peralatan",
      deviceTotal: "Total Perangkat",
      unit: "Satuan",
      unitValue: "Satuan: {unit}",
      actions: "Aksi",
      deviceDetails: "Detail perangkat",
      stockDetails: "Detail stok",
      stockHistory: "Catatan stok",
      viewStockHistory: "Lihat catatan stok",
      deviceInstanceDetails: "Detail instans perangkat",
      allDevices: "Semua perangkat",
      all: "Semua",
      statusLabel: "Status",
      sourceWarehouse: "Gudang Asal",
      owner: "Penerima / Penanggung Jawab",
      site: "Situs",
      sector: "Sektor",
      siteSector: "Situs / Sektor",
      sectorValue: "Sektor {value}",
      statusTime: "Waktu Status",
      deviceTracking: "Lacak perangkat",
      currentStock: "Stok Saat Ini",
      outboundPending: "Keluar, Belum Kembali",
      zeroEquipmentCount: "{count} material stok nol",
      stockStatus: "Status Stok",
      zeroWarehouseCount: "{count} gudang stok nol",
      restockWarehouseCount: "{count} gudang perlu restok",
      auxiliaryDetails: "Detail material",
      stockDistribution: "Distribusi Stok",
      stockTransactions: "Transaksi Stok",
      reorderPoint: "Batas Restok",
      notConfigured: "Belum diatur",
      lastUpdated: "Terakhir Diperbarui",
      documentRequest: "Dokumen Keluar / Permintaan",
      documentNumber: "Nomor Dokumen",
      requestNumber: "Nomor Permintaan",
      outboundQuantity: "Keluar",
      returnedQuantity: "Kembali",
      pendingQuantity: "Belum Kembali",
      operationTime: "Waktu Keluar",
      operationTimeGeneric: "Waktu Operasi",
      transactionType: "Jenis Operasi",
      quantity: "Jumlah",
      viewDocument: "Lihat dokumen",
      transactionTypes: {
        stock_in: "Barang Masuk",
        stock_out: "Barang Keluar",
        transfer: "Transfer",
        return: "Retur",
        adjustment: "Penyesuaian",
        damage: "Kerusakan",
      },
      status: {
        in_stock: "Di Gudang",
        issued: "Sudah Keluar",
        pending_inspection: "Menunggu Pemeriksaan",
        inspected: "Sudah Diperiksa",
        return_pending_receive: "Menunggu Penerimaan Retur",
        abnormal: "Bermasalah",
      },
      auxStatus: {
        stocked: "Tersedia",
        zero_stock: "Stok Nol",
        needs_restock: "Perlu Restok",
        partial_zero: "Sebagian Gudang Kosong",
      },
      auxFilter: {
        stocked: "Tersedia",
        zero_stock: "Stok Nol",
        needs_restock: "Perlu Restok",
        unconfigured: "Batas Restok Belum Diatur",
      },
    },
  },
  dashboard: {
    siteOverview: {
      title: "Ikhtisar Instalasi Situs",
      mapButton: "Lihat Peta Situs",
      cards: {
        survey: {
          title: "Situs Survei",
          desc: "Survei instalasi selesai / total situs",
        },
        planning: {
          title: "Situs Perencanaan",
          desc: "Perencanaan instalasi selesai / total situs",
        },
        installStarted: {
          title: "Instal Situs yang Dimulai",
          desc: "Pengikatan perangkat instalasi dimulai / total situs",
        },
        installed: {
          title: "Instal Situs Selesai",
          desc: "WO instalasi sudah dikirim / total situs",
        },
        partialOnline: {
          title: "Situs Online Sebagian",
          desc: "Sebagian perangkat instalasi online / total situs",
        },
        fullyOnline: {
          title: "Situs Online Penuh",
          desc: "Semua perangkat instalasi online / total situs",
        },
        partialActivated: {
          title: "Situs Aktif Sebagian",
          desc: "Sebagian perangkat instalasi aktif / total situs",
        },
        fullyActivated: {
          title: "Situs Aktif Penuh",
          desc: "Semua perangkat instalasi aktif / total situs",
        },
        online: {
          title: "Situs Online Penuh",
          desc: "Semua perangkat online / total situs",
        },
        activated: {
          title: "Situs Aktif Penuh",
          desc: "Semua perangkat aktif / total situs",
        },
        ssv: {
          title: "Situs SSV",
          desc: "Lulus SSV / total situs",
        },
      },
      deviceFraction: {
        online: "Perangkat online {numerator}/{denominator}",
        activated: "Perangkat aktif {numerator}/{denominator}",
      },
    },
    cellExpansionOverview: {
      title: "Ikhtisar Ekspansi Sel",
      subtitle: "Hanya menghitung sel/perangkat tambahan dan tidak mengubah progres instalasi",
      viewOrders: "Lihat WO Ekspansi",
      cards: {
        sites: { title: "Situs Ekspansi", desc: "Situs dengan WO ekspansi sel" },
        newCells: { title: "Sel/Perangkat Baru", desc: "Cakupan target baru ekspansi" },
        bound: { title: "Terikat", desc: "Slot perangkat baru sudah terikat SN" },
        online: { title: "Online", desc: "Penuh {full} · Sebagian {partial}" },
        activated: { title: "Aktif", desc: "Penuh {full} · Sebagian {partial}" },
        activeOrders: { title: "Berjalan", desc: "WO ekspansi belum selesai" },
        completedOrders: { title: "Selesai", desc: "WO ekspansi selesai" },
      },
    },
    installBreakdown: {
      title: "Grup Progres Situs",
      subtitle: "Lihat status instalasi, online, dan aktivasi berdasarkan grup bisnis",
      subtitleWithCategory: "Lihat status instalasi, online, dan aktivasi berdasarkan {category}",
      categoryPlaceholder: "Pilih dimensi grup",
      settings: "Pengaturan Grup",
      goSettings: "Buka Pengaturan Grup",
      viewSites: "Lihat Situs",
      loading: "Memuat...",
      empty: "Belum ada statistik grup",
      loadFailed: "Gagal memuat grup progres instalasi",
      unassigned: "Belum Dikelompokkan",
      knownCategories: {
        delivery_scope: "Cakupan Pengiriman",
      },
      columns: {
        group: "Grup",
        total: "Total Situs",
        installStarted: "Dimulai",
        installed: "Instalasi Selesai",
        notInstalled: "Belum Instalasi",
        partialOnline: "Online Sebagian",
        fullyOnline: "Online Penuh",
        partialActivated: "Aktif Sebagian",
        fullyActivated: "Aktif Penuh",
        ssv: "SSV Selesai",
        actions: "Aksi",
      },
      deviceFraction: {
        online: "Perangkat online {numerator}/{denominator}",
        activated: "Perangkat aktif {numerator}/{denominator}",
      },
    },
    siteTrend: {
      title: "Tren Peristiwa Situs",
      granularity: {
        day: "Sehari-hari",
        week: "Mingguan",
        month: "Bulanan",
      },
      valueMode: {
        incremental: "tambahan",
        cumulative: "Kumulatif",
      },
      chartMode: {
        auto: "Otomatis (Disarankan)",
        bar: "Bagan Batang",
        line: "Bagan Garis",
      },
      series: {
        install_started: "Instal Situs yang Dimulai",
        install_completed: "Instal Situs Selesai",
        online: "Situs Daring",
        activated: "Situs yang Diaktifkan",
        ssv: "Situs SSV",
      },
      empty: "Tidak ada data tren",
      loadFailed: "Gagal memuat tren situs",
    },
  },
  siteMap: {
    page: {
      title: "Peta Situs",
      subtitle:
        "Lihat distribusi situs berdasarkan wilayah dan status, dengan filter, penentuan posisi tertaut, dan navigasi detail cepat.",
    },
    headerTags: {
      loaded: "Memuat situs {count}",
      withCoordinates: "Dengan koordinat {count}",
      withoutCoordinates: "Tanpa koordinat {count}",
    },
    filters: {
      keywordPlaceholder: "Cari berdasarkan nama situs/kode/kota (otomatis)",
      statusPlaceholder: "Status situs",
      siteTypePlaceholder: "Jenis situs",
      provincePlaceholder: "Propinsi",
      cityPlaceholder: "Kota",
      districtPlaceholder: "Daerah",
      coordinatePlaceholder: "Status koordinat",
      ssvPlaceholder: "status SSV",
      onlyInView: "Hanya di area pandang peta saat ini",
      reset: "Mengatur ulang",
      refresh: "Segarkan Situs",
      coordinateOptions: {
        with: "Dengan koordinat saja",
        without: "Tanpa koordinat saja",
        all: "Semua",
      },
      ssvOptions: {
        all: "Semua SSV",
        passed: "Hanya lulus",
        failed: "Tidak lulus saja",
      },
    },
    tips: {
      filterHint:
        "Kata kunci, status, dan jenis menggunakan pencarian backend; filter wilayah, SSV, koordinat, dan area pandang adalah pemfilteran lokal instan.",
      truncatedHint:
        "Kumpulan data berukuran besar. Hanya situs {count} pertama yang dimuat. Harap persempit filter Anda.",
    },
    mapCard: {
      title: "Peta Distribusi Situs",
      filteredCount: "{count} yang difilter",
      markerCount: "Penanda peta {count}",
      fitAll: "Cocok untuk Semua Penanda",
      expandList: "Perluas Daftar",
      collapseList: "Ciutkan Daftar",
      siteList: "Daftar Situs",
      empty: "Tidak ada situs yang cocok dengan filter saat ini",
      ssvPassed: "SSV Lulus",
      ssvFailed: "SSV Tidak Lulus",
      hasCoordinates: "Memiliki Koordinat",
      noCoordinates: "Tidak Ada Koordinat",
      locate: "Menemukan",
      detail: "Detail",
      planning: "Perencanaan",
      legendTitle: "Legenda Status",
      noCoordinateHint:
        "Situs {count} dalam filter saat ini tidak memiliki koordinat dan hanya ditampilkan dalam daftar.",
    },
    status: {
      survey_pending: "Sedang Diinvestigasi",
      planning: "Di bawah Perencanaan",
      planned: "Perencanaan Selesai",
      construction: "Sedang dibangun",
      pending_online: "Menjadi Daring",
      online_pending_activation: "Sudah Online dan Menunggu untuk Diaktifkan",
      operational: "Sudah Diaktifkan",
      maintenance: "Sedang dalam Pemeliharaan",
      unknown: "Status Tidak Diketahui",
    },
    popup: {
      siteCode: "Kode Situs:",
      siteStatus: "Status Situs:",
      siteType: "Jenis Situs:",
      region: "Wilayah:",
      ssv: "SSV:",
      address: "Alamat:",
      passed: "Lulus",
      failed: "Tidak Lulus",
      regionUnset: "Wilayah tidak ditetapkan",
    },
    messages: {
      loadedSuccess: "Memuat situs {count}",
      loadFailed: "Gagal memuat peta situs",
      noCoordinateWarning:
        "Situs ini tidak memiliki koordinat dan tidak dapat ditemukan di peta",
      outOfViewWarning:
        "Situs ini tidak ada di area pandang peta saat ini. Sesuaikan filter dan coba lagi",
    },
  },
  siteList: {
    dashboardProgressFilterPrefix: "Filter dasbor: ",
    dashboardProgressFilters: {
      survey_done: "Situs Survei",
      planning_done: "Situs Perencanaan",
      install_started: "Situs Instalasi Dimulai",
      installed: "Situs Instalasi Selesai",
      partial_online: "Situs Online Sebagian",
      fully_online: "Situs Online Penuh",
      partial_activated: "Situs Aktif Sebagian",
      fully_activated: "Situs Aktif Penuh",
      ssv_passed: "Situs SSV",
    },
    actions: {
      more: "Tindakan Lainnya",
      rebuildProgress: "Hitung Ulang Siklus Hidup Situs",
    },
    batchEdit: {
      contractAmount: "Nilai Kontrak Situs",
      contractAmountMin:
        "Nilai kontrak situs pada baris {index} tidak boleh kurang dari 0",
    },
    progressRebuild: {
      confirm: {
        title: "Hitung Ulang Siklus Hidup Situs",
        message:
          "Ini akan menghitung ulang snapshot siklus hidup semua situs berdasarkan fakta valid saat ini. Gunakan untuk inisialisasi produksi, koreksi data historis, atau penelusuran masalah. Lanjutkan?",
        confirmButton: "Mulai Hitung Ulang",
      },
      messages: {
        success:
          "Perhitungan ulang selesai: meminta {requestedCount} situs, menghitung ulang {rebuiltCount}, melewati {skippedCount}.",
        failed: "Perhitungan ulang gagal: {message}",
      },
    },
  },
  sitePaymentSettings: {
    pageTitle: "Aturan Pembayaran Situs",
    actions: {
      refresh: "Segarkan",
      addRule: "Tambah Aturan",
      addProfile: "Tambah Profil Subkontraktor",
      copyDefaultRules: "Salin Aturan Default",
      save: "Simpan",
      delete: "Hapus",
    },
    summaryAlert:
      "Profil default berlaku untuk semua situs. Jika situs memiliki dimensi grup Subkontraktor, profil pembayaran subkontraktor yang cocok akan dipakai lebih dulu. Nilai berbasis rasio dapat dihitung dari nilai kontrak situs atau harga satuan situs di profil.",
    subcontractorMissingTip:
      "Kategori subkontraktor belum dikonfigurasi. Pilih kategori grup situs yang mewakili subkontraktor di Konfigurasi Sistem -> Eksekusi Work Order Web -> Tautan Penugasan Work Order, lalu kelola opsi subkontraktornya.",
    configVersion: "Versi Konfigurasi",
    configVersionTip:
      "Saat disimpan, versi saat ini akan langsung disimpan. Jika perlu kenaikan versi yang ketat, pemeriksaan konkurensi bisa ditambahkan nanti.",
    currency: "Mata Uang",
    currencyPlaceholder: "Contoh USD / CNY / UGX",
    currencyTip:
      "Pilih dari daftar preset, atau ketik kode mata uang kustom (akan diubah ke huruf besar saat disimpan).",
    rulesTitle: "Daftar Aturan",
    rulesTip:
      "Atur apakah sebuah node dibayar, rasio atau jumlah tetapnya, apakah bergantung pada approval akhir work order instalasi situs, dan aturan diskon warning di sini.",
    empty:
      'Belum ada aturan pembayaran. Klik "Tambah Aturan" di kanan atas untuk mulai.',
    emptyRules: "Profil ini belum memiliki aturan pembayaran.",
    defaultProfileName: "Profil Pembayaran Default",
    newProfileName: "Profil Pembayaran Subkontraktor Baru",
    profileCardTitle: "Profil Pembayaran {index}",
    scope: {
      default: "Profil Default",
      subcontractor: "Profil Subkontraktor",
      subcontractorWithName: "Subkontraktor: {name}",
    },
    ruleCardTitle: "Aturan {index}",
    fields: {
      profileName: "Nama Profil",
      scope: "Cakupan",
      subcontractor: "Subkontraktor",
      subcontractorPlaceholder: "Pilih subkontraktor",
      name: "Nama Aturan",
      namePlaceholder: "Contoh: Instalasi Selesai 40%",
      milestone: "Node",
      enabled: "Dibayar",
      ruleEnabled: "Dibayar",
      enabledOn: "Bayar",
      enabledOff: "Tidak Bayar",
      amountBase: "Dasar Nilai",
      amountBaseSite: "Nilai Kontrak Situs",
      amountBaseProfile: "Harga Satuan Situs Profil",
      profileAmount: "Harga Satuan Situs",
      amountType: "Jenis Nilai",
      amountTypeRatio: "Rasio",
      amountTypeFixed: "Nilai Tetap",
      amountRatio: "Rasio Pembayaran (%)",
      amountFixed: "Jumlah Pembayaran",
      requiresApprove: "Bergantung pada Approval Akhir Work Order Instalasi Situs",
      warningDiscount: "Diskon Warning",
      sortOrder: "Urutan",
      remark: "Catatan / Syarat",
      remarkPlaceholder:
        "Contoh: pembayaran hanya dapat diajukan setelah ada konfirmasi tertulis dari pelanggan",
      profileRemarkPlaceholder:
        "Contoh: subkontraktor ini memakai harga satuan kontrak baru",
    },
    messages: {
      loadFailed: "Gagal memuat aturan pembayaran situs",
      incompleteProfile:
        "Harap lengkapi nama profil, subkontraktor, dan harga satuan situs",
      incompleteRule: "Harap lengkapi nama aturan dan node terlebih dahulu",
      currencyTooLong: "Kode mata uang tidak boleh lebih dari 20 karakter",
      saveSuccess: "Berhasil disimpan",
      saveFailed: "Gagal menyimpan aturan pembayaran situs",
    },
  },
  siteBasicBatch: {
    batchUpdate: {
      contractAmountTip:
        "Alur yang disarankan: ekspor Excel dari daftar situs dengan filter, edit, lalu unggah kembali; situs dapat dicocokkan dengan site_id atau site_code, jalankan dry-run dulu lalu eksekusi. Kolom `contract_amount` juga didukung untuk memelihara nilai kontrak situs yang dipakai dalam perhitungan pembayaran berbasis rasio.",
    },
  },
  siteDetail: {
    pageTitle: "Detail Situs",
    common: {
      cancel: "Membatalkan",
      save: "Menyimpan",
    },
    actions: {
      back: "Kembali",
      surveyArchive: "Arsip Survei",
      openingArchive: "Arsip Instalasi Situs",
      createResurvey: "Buat survei ulang",
      createSurvey: "Buat Survei",
      createResurveyDisabledTip:
        "Situs ini telah melewatkan survei. Hanya admin/manajer yang dapat membuat perintah kerja survei ulang.",
    },
    basic: {
      title: "Informasi Situs Dasar",
      skipSurvey: "Lewati Survei",
      requireSurvey: "Perlu Survei Lagi",
      edit: "Sunting",
      delete: "Hapus Situs",
      siteName: "Nama Situs",
      siteCode: "Kode Situs",
      siteType: "Jenis",
      siteStatus: "Status",
      surveyRequirement: "Persyaratan Survei",
      surveyRequired: "Diperlukan Survei",
      surveyNotRequired: "Survei Tidak Diperlukan",
      ssvStatus: "SSV",
      ssvPassed: "Lulus",
      ssvNotPassed: "Tidak Lulus",
      address: "Alamat",
      region: "Wilayah",
      skipRecordTitle: "Lewati Rekam",
      skippedTag: "Dilewati Sebelumnya",
      skippedAt: "Waktu",
      skippedBy: "Operator",
      skippedReason: "Alasan",
      noReason: "Tidak disediakan",
      placeholder: "-",
    },
    milestones: {
      title: "Garis Waktu Pencapaian Penting",
      reachedCount: "Selesai {count}/{total}",
      subtitle:
        "Ditampilkan berdasarkan waktu kejadian pertama untuk membantu pengguna memahami kemajuan saat ini dengan cepat.",
      reached: "Dicapai",
      pending: "Tidak Terjadi",
      operator: "Operator",
      remark: "Catatan",
      approveWithProof: "Unggah Bukti dan Setujui",
      replaceProof: "Perbarui Bukti",
      approveSuccess: "Pencapaian berhasil ditandai",
      approveFailed: "Gagal memperbarui pencapaian",
      dialogTitle: "Unggah Bukti dan Konfirmasi Pencapaian",
      dialogNode: "Pencapaian",
      dialogFiles: "File Bukti",
      dialogConfirm: "Konfirmasi",
      dialogRemarkPlaceholder: "Catatan opsional untuk pencapaian ini",
      dialogUploadText: "Seret file ke sini atau klik untuk mengunggah",
      dialogFilesRequired: "Harap unggah minimal satu file bukti",
      installStarted: {
        label: "Penginstalan Dimulai",
        desc: "Pengikatan SN perangkat pertama",
      },
      installCompleted: {
        label: "Instal Selesai",
        desc: "Penyerahan perintah kerja instalasi situs pertama",
      },
      online: {
        label: "On line",
        desc: "Aktivasi perintah kerja instalasi situs pertama",
      },
      activated: {
        label: "Diaktifkan",
        desc: "Penyelesaian perintah kerja instalasi situs pertama",
      },
      ssv: {
        label: "SSV",
        desc: "Penyelesaian perintah kerja SSV pertama",
      },
      customerApproved: {
        label: "Persetujuan Pelanggan",
        desc: "Disetujui manual di Web setelah unggah bukti",
      },
      pac: {
        label: "PAC",
        desc: "Disetujui manual di Web setelah unggah bukti",
      },
    },
    editDialog: {
      title: "Edit Informasi Situs Dasar",
      siteCode: "Kode Situs",
      siteName: "Nama Situs",
      siteType: "Jenis",
      region: "Provinsi/Kota/Kabupaten",
      province: "Propinsi",
      city: "Kota",
      district: "Daerah",
      address: "Alamat",
      coordinates: "Koordinat",
      priority: "Prioritas",
      contact: "Kontak",
      contractAmount: "Nilai Kontrak Situs",
      remark: "Komentar",
      requiredPlaceholder: "Diperlukan",
      siteTypePlaceholder: "makro/dalam ruangan...",
      addressPlaceholder: "Alamat lengkap",
      latitudePlaceholder: "Lintang -90~90",
      longitudePlaceholder: "Bujur -180~180",
      contactName: "Nama",
      contactPhone: "Telepon",
      priorityOptions: {
        high: "Tinggi",
        normal: "Normal",
        low: "Rendah",
      },
    },
    payment: {
      title: "Catatan Pembayaran",
      profile: "Profil Pembayaran",
      defaultProfile: "Profil Pembayaran Default",
      subcontractor: "Subkontraktor",
      noSubcontractor: "Belum ditetapkan",
      contractAmount: "Nilai Kontrak",
      profileAmountBase: "Harga Satuan Profil",
      contractAmountMissing:
        "Profil pembayaran saat ini belum memiliki dasar nilai. Node pembayaran berbasis rasio belum bisa dihitung.",
      openingWorkOrder: "Perintah Kerja Instalasi Situs",
      noOpeningWorkOrder: "Belum ada perintah kerja instalasi situs",
      empty: "Belum ada aturan pembayaran",
      warningCount: "{count} item warning",
      originalAmount: "Nilai awal {amount}",
      columns: {
        rule: "Node Pembayaran",
        amount: "Jumlah",
        status: "Status",
        conditions: "Syarat",
      },
      status: {
        disabled: "Tidak Dibayar",
        pendingMilestone: "Menunggu Node",
        pendingWorkOrderApproval: "Menunggu Approval WO",
        pendingAmountBase: "Menunggu Nilai Dasar",
        ready: "Siap Ditagih",
      },
    },
    deleteDialog: {
      title: "Hapus Situs",
      warning:
        "Penghapusan bersifat fisik dan hanya diperbolehkan untuk situs tanpa data terkait. Tindakan ini tidak dapat dibatalkan.",
      siteName: "Nama Situs",
      siteCode: "Kode Situs",
      confirmCode: "Konfirmasikan Kode Situs",
      confirmCodePlaceholder:
        "Masukkan kode situs untuk mengonfirmasi penghapusan",
      confirmDelete: "Konfirmasi Hapus",
    },
    workOrder: {
      title: "Perintah Kerja Saat Ini",
      historyButton: "Lihat Riwayat",
      historyTitle: "Riwayat Perintah Kerja",
      close: "Menutup",
      empty: "Tidak ada perintah kerja aktif",
      view: "Melihat",
      loadFailed: "Gagal memuat perintah kerja",
      loadHistoryFailed: "Gagal memuat perintah kerja riwayat",
      columns: {
        title: "Judul",
        type: "Jenis",
        status: "Status",
        priority: "Prioritas",
        assignee: "Penerima tugas",
        assignedAt: "Ditugaskan Pada",
        completedAt: "Selesai Pada",
        action: "Tindakan",
      },
      typeMap: {
        INSPECTION: "Inspeksi",
        INSTALLATION: "Instalasi",
        MAINTENANCE: "Pemeliharaan",
        REPAIR: "Memperbaiki",
      },
      statusMap: {
        PENDING: "Tugas yang Tertunda",
        ASSIGNED: "Ditugaskan",
        ACCEPTED: "Diterima",
        IN_PROGRESS: "Sedang Berlangsung",
        SUBMITTED: "Dikirim",
        UNDER_REVIEW: "Sedang Ditinjau",
        APPROVED: "Disetujui",
        REJECTED: "Ditolak",
        COMPLETED: "Selesai",
        CANCELLED: "Dibatalkan",
      },
    },
    device: {
      title: "Perangkat Online / Status Aktivasi",
      checkedAt: "Terakhir diperiksa: {time}",
      refresh: "Segarkan Status",
      refreshCooldown: "Segarkan Status ({seconds}s)",
      refreshWait: "Harap tunggu {seconds}s sebelum menyegarkan kembali",
      empty: "Tidak ada catatan perangkat yang terikat",
      loadFailed: "Gagal memuat status perangkat",
      columns: {
        sn: "Perangkat SN",
        type: "Jenis Perangkat",
        model: "Model Perangkat",
        sectorInfo: "Info Sektor",
        onlineStatus: "Status Daring",
        activeStatus: "Status Aktivasi",
        installer: "Pemasang",
        boundAt: "Terikat Pada",
      },
      sectorInfoValue: "Sektor {sector} / Band {band} / Sel {cell}",
      onlineCurrent: "Saat ini Daring",
      onlineOffline: "Saat ini Offline",
      activeCurrent: "Saat Ini Diaktifkan",
      activeInactive: "Saat Ini Tidak Diaktifkan",
      pendingCheck: "Pemeriksaan Tertunda",
      unbound: "Tidak Terikat",
      everOnline: "Pernah Daring",
      neverOnline: "Tidak Pernah Daring",
      everActive: "Pernah Diaktifkan",
      neverActive: "Tidak Pernah Diaktifkan",
    },
    priorityMap: {
      HIGH: "Tinggi",
      NORMAL: "Normal",
      LOW: "Rendah",
    },
    messages: {
      loadSiteFailed: "Gagal memuat detail situs",
      networkError: "Kesalahan jaringan",
      operationFailed: "Operasi gagal: {detail}",
      siteNameRequired: "Silakan masukkan nama situs",
      saveSuccess: "Berhasil disimpan",
      saveFailed: "Gagal menyimpan",
      relatedDataCannotDelete:
        "Situs memiliki data terkait dan tidak dapat dihapus",
      codeMismatch: "Kode situs tidak cocok",
      deleteSuccess: "Berhasil dihapus",
      deleteFailed: "Penghapusan gagal",
      surveySkipSuccess: "Tahap survei dilewati",
      surveyRequireSuccess: "Situs diatur kembali untuk memerlukan survei",
      noSurveyArchive: "Tidak ada arsip survei untuk situs ini",
      getSurveyArchiveFailed: "Gagal mendapatkan arsip survei",
      noOpeningArchive: "Tidak ada arsip instalasi situs untuk situs ini",
      getOpeningArchiveFailed: "Gagal mendapatkan arsip instalasi situs",
    },
    prompts: {
      skipSurveyMessage:
        "Tandai situs ini sebagai survei tidak diperlukan dan lanjutkan ke perencanaan. Alasan (opsional):",
      skipSurveyTitle: "Lewati Survei",
      requireSurveyMessage:
        "Atur situs ini kembali ke survei yang diperlukan dan kembalikan ke survey_pending. Alasan (opsional):",
      requireSurveyTitle: "Perlu Survei Lagi",
      confirmSkip: "Konfirmasi Lewati",
      confirmRequire: "Mengonfirmasi",
      optionalPlaceholder: "Opsional",
      cancel: "Membatalkan",
      confirm: "Mengonfirmasi",
      createResurveyTitle: "Buat survei ulang",
      createResurveyConfirm:
        "Buat perintah kerja survei ulang? Setelah pembuatan, situs akan diatur kembali agar memerlukan survei (lewati catatan dipertahankan).{skipInfo}",
    },
    deleteCheck: {
      blockedPrefix: "Data terkait ada, penghapusan tidak diperbolehkan",
      counts: {
        work_orders: "Perintah Kerja",
        site_inspections: "Catatan Inspeksi",
        inspections: "Catatan Inspeksi Warisan",
        site_surveys: "Catatan Survei",
        site_survey_archives: "Arsip Survei",
        site_opening_archives: "Arsip Instalasi Situs",
        site_ssv_archives: "Arsip SSV",
        equipment_binding_history: "Sejarah Pengikatan Peralatan",
        site_planning: "Versi Perencanaan",
        site_planning_cells: "Sel Perencanaan",
        base_station_devices: "Perangkat Stasiun Pangkalan",
        template_bindings: "Pengikatan Templat",
      },
    },
    requireSurveyDisableTip: {
      statusOnly: "Hanya tersedia dalam status perencanaan",
      noPlanningVersion: "Hanya tersedia bila tidak ada versi perencanaan",
    },
  },
  workOrderList: {
    pageTitle: "Daftar Perintah Kerja",
    filters: {
      searchPlaceholder:
        "Cari judul/deskripsi/nama situs/kode (aktif otomatis)",
      statusPlaceholder: "Status",
      typePlaceholder: "Jenis",
      multiStatusActive: "Filter Multi-status",
      multiStatusTooltip: "Sedang memfilter beberapa status: {statuses}",
    },
    sort: {
      button: "Urutkan",
      fieldLabel: "Bidang",
      directionLabel: "Arah",
      reset: "Kembalikan Default",
      orders: {
        desc: "Menurun",
        asc: "Menaik",
      },
      fields: {
        createdAt: "Waktu Dibuat",
        updatedAt: "Waktu Diperbarui",
        assignedAt: "Waktu Penugasan",
        dueDate: "Batas Waktu",
        priority: "Prioritas",
        status: "Status",
        type: "Jenis",
        siteCode: "Kode Situs",
        siteName: "Nama Situs",
      },
    },
    table: {
      title: "Judul",
      type: "Jenis",
      site: "Situs",
      assignee: "Ditugaskan Ke",
      reviewer: "Peninjau",
      priority: "Prioritas",
      status: "Status",
      assignedAt: "Waktu Penugasan",
      actions: "Tindakan",
      creator: "Dibuat Oleh",
    },
    actions: {
      refresh: "Segarkan",
      export: "Ekspor",
      create: "Buat Perintah Kerja",
      clearSelection: "Batalkan Pilihan",
      batchAssign: "Tugaskan Ulang Massal",
      batchPriority: "Ubah Prioritas Massal",
      batchVoid: "Batalkan Massal",
      batchDelete: "Hapus Massal",
      edit: "Edit",
      review: "Tinjau",
      void: "Batalkan",
      delete: "Hapus",
      save: "Simpan",
      confirm: "Konfirmasi",
      confirmAssign: "Konfirmasi Penugasan",
      confirmUpdate: "Konfirmasi Perubahan",
      confirmContinueCreate: "Tetap Buat",
      voidConfirm: "Konfirmasi Pembatalan",
      uploadExpansionLld: "Unggah LLD Target",
      prepareExpansionLld: "Siapkan LLD Target",
      reprepareExpansionLld: "Siapkan Ulang",
      downloadExpansionDraft: "Unduh Draf",
      useExpansionLld: "Gunakan LLD Target Ini",
    },
    dialogs: {
      createTitle: "Buat Perintah Kerja",
      duplicateTitle: "Situs Ini Sudah Memiliki Perintah Kerja Instalasi",
      duplicateAlert:
        "Anda masih bisa melanjutkan, tetapi hindari pengiriman perintah ganda.",
      siteLabel: "Situs",
      editTitle: "Edit Perintah Kerja",
      batchAssignTitle: "Tugaskan Ulang Massal",
      batchPriorityTitle: "Ubah Prioritas Massal",
      batchResultTitle: "Hasil Operasi Massal",
    },
    expansionDialog: {
      title: "Siapkan LLD Target",
      bodyHint:
        "Draf diekspor dari LLD saat ini. Pertahankan sel yang ada, tambahkan sel ekspansi, lalu unggah LLD target lengkap.",
      empty:
        "Unduh draf dan unggah LLD target lengkap. Pratinjau perbedaan akan muncul di sini.",
      newCells: "{count} target sel/perangkat baru",
      steps: {
        download: "Unduh Draf",
        upload: "Unggah LLD Lengkap",
        preview: "Pratinjau Perbedaan",
        use: "Gunakan LLD Target",
      },
    },
    batch: {
      selectedCount: "{count} perintah kerja dipilih",
      successHandled: "{count} perintah kerja berhasil diproses",
      failedHandled: "{count} perintah kerja gagal",
      errorDetails: "Detail Kegagalan",
    },
    form: {
      site: "Situs",
      type: "Jenis",
      assignee: "Ditugaskan Ke",
      title: "Judul",
      inspectionTemplate: "Templat Inspeksi",
      replacementTargets: "Slot Penggantian",
      replacementTargetHint:
        "Dihasilkan dari slot perencanaan situs (sector_id + band). Mendukung pilihan ganda.",
      replacementTargetOption: "Sektor {sectorId} / Band {band}",
      expansionLld: "LLD Target",
      expansionLldReady: "Siap",
      expansionLldPending: "Belum Siap",
      expansionLldNotPrepared: "LLD target belum disiapkan",
      expansionLldHint:
        "Unduh LLD saat ini sebagai draf, tambahkan sel baru, lalu unggah LLD target lengkap.",
      cellExpansionTemplateHint:
        "Ekspansi sel memakai ulang templat instalasi situs baru. Sistem terlebih dahulu memakai templat dari perintah kerja instalasi valid terbaru di situs ini dan hanya membuat item pemeriksaan untuk sel/perangkat baru.",
      expansionPreviewTitle:
        "Pratinjau lolos: saat ini {currentSectors} sektor / {currentCells} sel → target {targetSectors} sektor / {targetCells} sel, {newCells} target sel/perangkat baru",
      expansionCellLabel:
        "Sektor {physicalSector} / LCID {localCellId} / Band {band}",
      priority: "Prioritas",
      dueDate: "Batas Waktu",
      description: "Deskripsi",
      loading: "Memuat...",
      loadFailed: "Gagal memuat",
      noTemplates: "Tidak ada templat inspeksi yang tersedia",
      templateLoadFailed: "Gagal memuat templat inspeksi",
      placeholders: {
        siteSearch: "Masukkan nama atau kode situs untuk mencari",
        selectType: "Pilih jenis",
        selectAssignee: "Pilih petugas",
        enterTitle: "Masukkan judul",
        template: "Pilih templat inspeksi",
        openingTemplateRequired: "Pilih templat inspeksi",
        replacementTargets: "Pilih slot penggantian (bisa lebih dari satu)",
        selectPriority: "Pilih prioritas",
        description: "Deskripsi perintah kerja opsional",
      },
    },
    tooltips: {
      duplicatePhotos:
        "Foto duplikat terdeteksi. Buka meja peninjauan untuk melihat sumbernya.",
      similarPhotos:
        "Foto sangat mirip terdeteksi. Buka meja peninjauan untuk memverifikasi.",
    },
    voidMeta: {
      label: "Dibatalkan",
      noReason: "Tanpa alasan",
    },
    prefill: {
      genericTitle: "Perintah Kerja",
      resurveyPrefix: "Survei Ulang-",
    },
    statuses: {
      PENDING: "Menunggu Penugasan",
      ACTIVE: "Sudah Ditugaskan",
      SUBMITTED: "Sudah Dikirim",
      UNDER_REVIEW: "Sedang Ditinjau",
      APPROVED: "Disetujui / Menunggu Online",
      ACTIVATED: "Tahap Online",
      REJECTED: "Ditolak",
      COMPLETED: "Selesai",
      VOIDED: "Dibatalkan",
    },
    statusOverrides: {
      APPROVED: "Siap Online",
      ACTIVATED: "Sudah Online, Menunggu Aktivasi",
      COMPLETED: "Sudah Diaktifkan",
    },
    siteStatuses: {
      survey_pending: "Menunggu Survei",
      planning: "Perencanaan",
      construction: "Konstruksi",
      pending_online: "Siap Online",
      online_pending_activation: "Online, Menunggu Aktivasi",
      operational: "Operasional",
      maintenance: "Pemeliharaan",
      decommissioned: "Dinonaktifkan",
      cancelled: "Dibatalkan",
    },
    types: {
      opening_inspection: "Instalasi Situs Baru",
      equipment_replacement: "Penggantian Peralatan",
      cell_expansion: "Ekspansi Sel",
      maintenance: "Inspeksi Pemeliharaan",
      power_issue: "Masalah Daya",
      transmission_issue: "Masalah Transmisi",
      gps_issue: "Masalah GPS",
      signal_issue: "Masalah Sinyal",
      site_survey: "Survei Situs",
      ssv: "Penerimaan SSV",
    },
    priorities: {
      low: "Rendah",
      normal: "Normal",
      high: "Tinggi",
      urgent: "Mendesak",
    },
    progress: {
      stateLabels: {
        current: "Tahap Saat Ini",
        done: "Tahap Selesai",
        pending: "Tahap Berikutnya",
        rejected: "Berhenti di Tahap Penolakan",
        muted: "Alur Dihentikan",
      },
      stages: {
        pending: {
          title: "Menunggu Penugasan",
          desc: "Perintah kerja sudah dibuat dan sedang menunggu petugas.",
        },
        active: {
          title: "Sudah Ditugaskan",
          desc: "Perintah kerja sudah ditugaskan dan menunggu penanganan lapangan.",
        },
        submitted: {
          title: "Sudah Dikirim",
          desc: "Pekerjaan lapangan selesai dan perintah kerja sudah dikirim untuk ditinjau.",
        },
        review: {
          title: "Tahap Tinjau",
          desc: "Perintah kerja sedang ditinjau atau menunggu hasil tinjauan.",
        },
        reviewActivation: {
          title: "Tahap Tinjau / Online",
          desc: "Setelah dikirim, perintah kerja masuk peninjauan; setelah lolos, proses online dan aktivasi dilanjutkan.",
        },
        completed: {
          title: "Selesai",
          desc: "Alur perintah kerja sudah berakhir.",
        },
        activated: {
          title: "Sudah Diaktifkan",
          desc: "Situs sudah online dan aktif, lalu perintah kerja ditutup.",
        },
      },
    },
    siteSearch: {
      loading: "Sedang mencari situs...",
      noMatch: "Tidak ada situs yang cocok",
      inputHint: "Masukkan nama atau kode situs untuk mencari",
      cellExpansionInputHint:
        "Cari situs yang sudah terpasang berdasarkan nama atau kode",
      cellExpansionNoMatch:
        "Tidak ada situs terpasang yang bisa dibuatkan ekspansi sel",
      cellExpansionScopeHint:
        "Ekspansi sel hanya mendukung situs siap online, online menunggu aktivasi, operasional, atau pemeliharaan.",
      loadingHint: "Sedang mencari situs, harap tunggu.",
      noKeywordResults:
        'Tidak ada situs yang cocok dengan "{keyword}". Coba nama situs, kode, atau kata kunci yang lebih pendek.',
      totalTruncated:
        "{total} situs cocok. Saat ini hanya menampilkan {visible} teratas, silakan persempit kata kunci.",
      foundCount: "{count} situs cocok ditemukan. Anda bisa memilih langsung.",
      defaultHint:
        "Saat dropdown dibuka, hanya sedikit saran yang ditampilkan. Masukkan nama atau kode situs untuk mencari lebih banyak.",
      suggestTruncated:
        "Saat ini hanya menampilkan {visible} saran teratas. Masukkan nama atau kode situs untuk mencari semua situs yang terlihat.",
      visibleCount:
        "Saat ini menampilkan {visible} situs saran. Anda bisa memilih langsung atau lanjut mencari.",
    },
    validation: {
      selectReplacementTarget: "Silakan pilih slot penggantian",
      uploadExpansionLld: "Silakan unggah dan lolos pratinjau LLD target",
      selectSite: "Silakan pilih situs",
      selectType: "Silakan pilih jenis",
      selectAssignee: "Silakan pilih petugas",
      selectTemplate: "Silakan pilih templat inspeksi",
      selectOpeningTemplate:
        "Perintah kerja ekspansi sel wajib memilih templat instalasi situs baru",
      cellExpansionSiteRequired:
        "Ekspansi sel hanya dapat memilih situs yang sudah terpasang atau online",
      enterTitle: "Silakan masukkan judul",
      selectNewStatus: "Silakan pilih status baru",
      selectAssigneeForBatch: "Silakan pilih petugas",
      selectPriorityForBatch: "Silakan pilih prioritas",
      voidReasonPlaceholder: "Masukkan alasan pembatalan minimal 5 karakter",
      voidReasonMin: "Alasan pembatalan minimal 5 karakter",
      voidReasonMax: "Alasan pembatalan tidak boleh lebih dari 200 karakter",
    },
    prompts: {
      deleteSingleTitle: "Konfirmasi Hapus",
      deleteSingleMessage:
        'Hapus perintah kerja "{title}"? Tindakan ini tidak bisa dibatalkan.',
      voidReasonNote:
        "Pembatalan akan menyimpan perintah kerja dan catatan inspeksi, tetapi membekukannya menjadi hanya-baca. Jika masih ada SN tingkat perangkat yang terikat, sistem akan menolak pembatalan. Silakan masukkan alasannya:",
      voidSingleTitle: "Konfirmasi Pembatalan Perintah Kerja",
      voidSingleMessage: 'Batalkan perintah kerja "{title}"?',
      voidBatchTitle: "Konfirmasi Pembatalan Massal",
      voidBatchMessage: "Batalkan {count} perintah kerja yang dipilih?",
      deleteBatchTitle: "Konfirmasi Hapus Massal",
      deleteBatchMessage: "Hapus {count} perintah kerja yang dipilih?",
      deleteBatchNote:
        'Catatan: hanya perintah kerja berstatus "Menunggu Penugasan" atau "Sudah Ditugaskan" yang bisa dihapus.',
      assignBatchTitle: "Konfirmasi Penugasan Ulang Massal",
      assignBatchMessage: "Tugaskan ulang {count} perintah kerja yang dipilih?",
      assignBatchNote:
        'Catatan: hanya perintah kerja berstatus "Menunggu Penugasan" atau "Sudah Ditugaskan" yang bisa ditugaskan ulang.',
    },
    messages: {
      loadFailed: "Gagal memuat perintah kerja",
      planningNotFound:
        "Perencanaan situs tidak ditemukan atau Anda tidak memiliki izin, sehingga slot penggantian tidak bisa dimuat",
      replacementTargetRequired:
        "Perintah kerja penggantian peralatan harus memilih slot penggantian",
      expansionTargetRequired:
        "Perintah kerja ekspansi sel harus mengunggah LLD target terlebih dahulu",
      expansionPreviewSuccess: "Pratinjau LLD target lolos",
      expansionPreviewFailed: "Pratinjau LLD target gagal",
      expansionDraftDownloaded: "Draf LLD target telah diunduh",
      expansionDraftDownloadFailed: "Gagal mengunduh draf",
      cellExpansionSiteNotAllowed:
        "Status situs saat ini {status}, sehingga ekspansi sel tidak dapat dibuat. Untuk situs yang belum terpasang, gunakan perintah kerja instalasi situs baru atau kelola LLD target langsung.",
      templateLoadFailed: "Gagal memuat templat inspeksi: {message}",
      templateLoadFailedFallback:
        "Muat ulang halaman dan coba lagi, atau minta administrator memeriksa izin templat.",
      createSuccess: "Berhasil dibuat",
      createFailed: "Gagal membuat",
      updateSuccess: "Berhasil diperbarui",
      updateFailed: "Gagal memperbarui",
      deleteSuccess: "Berhasil dihapus",
      deleteFailed: "Gagal menghapus",
      voidSuccess: "Perintah kerja dibatalkan",
      voidFailed: "Gagal membatalkan",
      voidBlockedTitle: "Perintah kerja belum dapat dibatalkan",
      voidBlockedFallback:
        "Perintah kerja ini belum memenuhi syarat pembatalan. Periksa perangkat yang masih terikat atau arsip serah terima yang sudah dibuat.",
      batchDeleteSuccess: "Berhasil menghapus {count} perintah kerja",
      batchDeleteFailed: "Gagal menghapus secara massal",
      batchVoidSuccess: "Berhasil membatalkan {count} perintah kerja",
      batchVoidFailed: "Gagal membatalkan secara massal",
      batchStatusSuccess: "Berhasil mengubah status {count} perintah kerja",
      batchStatusFailed: "Gagal mengubah status secara massal",
      batchAssignSuccess: "Berhasil menugaskan ulang {count} perintah kerja",
      batchAssignFailed: "Gagal menugaskan ulang secara massal",
      batchPrioritySuccess:
        "Berhasil mengubah prioritas {count} perintah kerja",
      batchPriorityFailed: "Gagal mengubah prioritas secara massal",
      exportEmpty: "Tidak ada perintah kerja yang bisa diekspor",
      exportSuccess: "Ekspor berhasil",
      exportFailed: "Ekspor gagal",
      exportFailedWithReason: "Ekspor gagal: {message}",
    },
  },
  operation: {
    moduleDefault: "Sistem",
    pageDefault: "Halaman",
    actionOpenPage: "Buka halaman",
    objectTypePage: "Halaman",
  },
  workOrderReview: {
    labels: {
      itemReviewTitle: "Tinjauan Item Pemeriksaan",
      itemDetailTitle: "Detail Item Pemeriksaan",
      checkItemPrefix: "Item pemeriksaan:",
      itemName: "Item Pemeriksaan",
      scope: "Cakupan",
      type: "Jenis",
      status: "Status",
      completedAt: "Waktu Selesai",
      reviewStatus: "Status Review",
      reviewComments: "Catatan Review",
      onsiteRemark: "Catatan Lapangan",
      aiSuggestion: "Saran AI",
      suggestedConclusion: "Kesimpulan Saran",
      suggestedScore: "Skor Saran",
      confidence: "Tingkat Keyakinan",
      analysisPoints: "Poin Analisis",
      missingEvidence: "Bukti Kurang / Perlu Ditambah",
      contentColumn: "Konten",
      duplicatePhotosColumn: "Foto Duplikat",
      similarPhotosColumn: "Foto Mirip",
      fieldDataSection: "Data Field & Foto Field",
      photoSection: "Foto Terkait ({count})",
      fieldName: "Nama Field",
      fieldValue: "Nilai",
      unit: "Satuan",
      photo: "Foto",
      noSubmittedContent:
        "Item pemeriksaan ini belum memiliki konten yang dikirim",
      cellExpansionInfo: "Info Ekspansi Sel",
      expansionSector: "Sektor / Sel",
      targetPlanningVersion: "Versi Perencanaan Target",
      newSlots: "Sel/Perangkat Baru",
    },
    filters: {
      reviewList: {
        all: "Semua",
        pending: "Lihat yang perlu ditinjau",
      },
      scope: {
        all: "Semua cakupan",
        site: "Situs",
        sector: "Sektor",
        cell: "Sel",
        equipment: "Peralatan",
      },
      distance: {
        all: "Semua jarak",
        exceeded: "Hanya melebihi jarak",
      },
    },
    summary: {
      passCount: "Lulus {count}",
      warningCount: "Peringatan {count}",
      failCount: "Tidak Lulus {count}",
      pendingCount: "Menunggu {count}",
    },
    actions: {
      aiCheckBatch: "Inspeksi AI Sekali Klik",
      aiCheckMissing: "Hanya yang belum dibuat",
      aiCheckAll: "Semua",
    },
    scope: {
      sectorTooltip: "Sektor {sector}",
      cellTooltip: "Sektor {sector} / Band {band} / Sel {cell}",
    },
    hints: {
      voided: {
        text: "Perintah kerja dibatalkan",
        tooltip:
          "Perintah kerja ini sudah dibatalkan, dan semua aksi review/edit dibekukan.",
      },
      pending: {
        text: "{count} belum ditinjau",
        tooltip:
          "Masih ada {count} item pemeriksaan yang belum ditinjau. Selesaikan semua review item sebelum review akhir.",
      },
      failed: {
        text: "Ada item tidak lulus",
        tooltip:
          "Ada item pemeriksaan yang tidak lulus, sehingga perintah kerja tidak bisa disetujui.",
      },
      ready: {
        text: "Siap untuk review akhir",
        tooltip:
          "Semua item pemeriksaan sudah ditinjau dan siap untuk review akhir.",
      },
    },
    messages: {
      snCopied: "SN berhasil disalin ke clipboard",
      copyFailed: "Gagal menyalin",
      itemReviewBlockedByVoided:
        "Perintah kerja dibatalkan, tidak dapat melanjutkan review item pemeriksaan",
      itemReviewBlockedByStatus:
        "Item pemeriksaan ini belum disubmit (saat ini: {status}) dan tidak bisa direview",
      submitSuccess: "Berhasil dikirim",
      submitFailed: "Gagal mengirim",
      similarSourceMissing:
        "Foto ini tidak memiliki ID sumber serupa untuk dibuka",
    },
    buttons: {
      cancel: "Batal",
      submit: "Kirim",
      close: "Tutup",
      pass: "Lulus",
      warning: "Peringatan",
      fail: "Tidak Lulus",
    },
    statuses: {
      pending: "Menunggu Review",
      running: "Sedang dibuat",
      failed: "Gagal",
      generated: "Sudah dibuat",
      notGenerated: "Belum dibuat",
    },
    types: {
      photo: "Foto",
      data: "Data",
      both: "Data + Foto",
    },
    content: {
      dataCount: "Data: {count}",
      photoCount: "Foto: {count}",
      empty: "Tidak ada konten",
      duplicateCount: "Duplikat {count}",
      similarCount: "Mirip {count}",
    },
    fieldIssues: {
      columnLabel: "Catatan Masalah",
      placeholder: "Hanya isi untuk field yang memang bermasalah",
      autoSummaryTitle: "Pratinjau Ringkasan Otomatis Masalah Field",
      autoSummaryHint:
        "Ringkasan ini hanya-baca dan akan otomatis digabung ke catatan review akhir saat submit.",
      countTag: "{count}",
      commentPlaceholderOptional: "Masukkan komentar review (opsional)",
      commentPlaceholderSupplementary:
        "Opsional: tambahkan catatan keseluruhan",
      commentPlaceholderRequired: "Masukkan komentar review (wajib)",
      submitWarning:
        "Masukkan komentar review atau isi minimal satu catatan masalah field",
      summaryTitle: "Ringkasan Masalah Field:",
      supplementTitle: "Catatan Tambahan:",
      summaryFallbackField: "Field {index}",
    },
    fieldReview: {
      helpTitle: "Deskripsi/catatan {label}",
      historyButton: "Riwayat",
      valueLabel: "Nilai: ",
      unitLabel: "Satuan: ",
      resultLabel: "Review Field: ",
      commentLabel: "Catatan Field: ",
      currentValueLabel: "Nilai Saat Ini",
      emptyValueLabel: "Nilai Kosong",
      actionTitle: "Keputusan Field",
      commentTitle: "Catatan Review",
      commentEmpty: "Belum ada catatan field",
      requiredTag: "Wajib",
      requiredMissingTag: "Wajib Belum Diisi",
      emptyTag: "Belum Diisi",
      commentMissingTag: "Perlu Catatan",
      results: {
        pass: "Lulus",
        warning: "Peringatan",
        fail: "Tidak Lulus",
        pending: "Belum Ditinjau",
      },
      summary: {
        total: "Field {count}",
        missing: "Kosong {count}",
        pending: "Menunggu {count}",
        warning: "Peringatan {count}",
        fail: "Tidak Lulus {count}",
      },
      historyDrawerTitle: "Riwayat Review Field (Semua Putaran)",
      historyFieldLabel: "Field: {field}",
      historyEmpty: "Belum ada riwayat",
      historyRoundTitle: "Putaran {round} ({count} catatan)",
      historyOpinionLabel: "Pendapat: ",
      historyFieldCommentLabel: "Catatan Field: ",
      fullWidthColon: ": ",
      historyContextMissing:
        "Konteks item pemeriksaan tidak lengkap, riwayat field tidak bisa ditampilkan",
      historyLoadFailed: "Gagal memuat riwayat review field",
      historyErrors: {
        inspectionNotFound: "Data inspeksi tidak ditemukan",
        itemNotFound: "Item pemeriksaan tidak ditemukan",
        accessDenied: "Tidak memiliki izin mengakses data inspeksi ini",
        permissionDenied: "Tidak memiliki izin melihat riwayat review field",
      },
      confirm: {
        pendingLine:
          "Masih ada {count} field yang belum ditinjau. Alur tetap lanjut berdasarkan kesimpulan item pemeriksaan.",
        conflictLine:
          "Kesimpulan manual item pemeriksaan ({action}) bertentangan dengan agregasi field ({autoResult}).",
        firstQuestion: "Yakin ingin melanjutkan submit?",
        firstTitle: "Konfirmasi Pertama",
        continueButton: "Lanjut",
        secondMessage:
          "Konfirmasi kedua: submit dengan hasil saat ini sebagai hasil akhir?",
        secondTitle: "Konfirmasi Kedua",
        submitButton: "Konfirmasi Submit",
      },
      errors: {
        manualOverrideRequired:
          "Kesimpulan manual item pemeriksaan ({action}) bertentangan dengan agregasi field ({autoResult}). Konfirmasi override manual lalu coba lagi.",
        confirmRequired:
          "Pengiriman ini memiliki risiko konflik. Selesaikan konfirmasi kedua lalu coba lagi.",
        confirmRequiredWithPending:
          "Masih ada {count} field yang belum ditinjau. Selesaikan konfirmasi kedua lalu coba lagi.",
        confirmRequiredWithPendingAndConflict:
          "Masih ada {count} field belum ditinjau dan kesimpulan item bertentangan dengan agregasi field. Selesaikan konfirmasi kedua lalu coba lagi.",
        itemNotCompleted:
          "Item pemeriksaan belum selesai submit dan tidak dapat direview (status saat ini: {status})",
        commentRequired:
          "Masukkan komentar review atau isi minimal satu catatan masalah field",
        itemNotFound: "Item pemeriksaan tidak ditemukan",
        inspectionNotFound: "Data inspeksi tidak ditemukan",
        permissionDenied: "Tidak memiliki izin untuk me-review item pemeriksaan ini",
      },
    },
    itemReviewHistory: {
      button: "Riwayat Review Item (Semua Putaran)",
      drawerTitle: "Riwayat Review Item (Semua Putaran)",
      empty: "Belum ada riwayat",
      roundTitle: "Putaran {round} ({count} catatan)",
      manualOverrideTag: "Override Manual",
      confirmOverrideTag: "Konfirmasi Kedua",
      autoResultLabel: "Agregasi Field: {result}",
      pendingCountLabel: "Field Belum Ditinjau: {count}",
      opinionLabel: "Pendapat: ",
      fieldDetailsTitle: "Detail Review Field",
      fieldLabelFallback: "Field Tanpa Nama",
      contextMissing:
        "Konteks item pemeriksaan tidak lengkap, riwayat review item tidak bisa ditampilkan",
      loadFailed: "Gagal memuat riwayat review item pemeriksaan",
      errors: {
        inspectionNotFound: "Data inspeksi tidak ditemukan",
        itemNotFound: "Item pemeriksaan tidak ditemukan",
        accessDenied: "Tidak memiliki izin mengakses data inspeksi ini",
        permissionDenied: "Tidak memiliki izin melihat riwayat review item pemeriksaan",
      },
    },
  },
  workOrderPhotoDetail: {
    title: "Detail Foto Serupa",
    buttons: {
      back: "Kembali",
      refresh: "Muat Ulang",
    },
    labels: {
      photoId: "ID Foto",
      fileName: "Nama File",
      takenAt: "Waktu Ambil",
      fileSize: "Ukuran File",
      latitude: "Lintang",
      longitude: "Bujur",
      gpsAccuracy: "Akurasi GPS",
      address: "Alamat",
      site: "Situs",
      workOrder: "Perintah Kerja",
      checkItem: "Item Pemeriksaan",
      field: "Field",
      sourcePhotoId: "ID Foto Sumber",
    },
    imageLoadFailed: "Gagal memuat gambar",
    imageNotFound: "URL gambar tidak ditemukan",
    detailNotFound: "Detail foto tidak ditemukan",
    status: {
      unknown: "Tidak diketahui",
    },
    siteText: {
      idFallback: "ID Situs:{siteId}",
      withId: "{siteName} (ID:{siteId})",
    },
    workOrderText: {
      idFallback: "ID Perintah Kerja:{workOrderId}",
      typePart: "Tipe:{type}",
      statusPart: "Status:{status}",
    },
    messages: {
      loadFailed: "Gagal memuat detail foto",
    },
  },
  route: {
    Login: "Login",
    Dashboard: "Dasbor",
    InventoryMgmt: "Manajemen Inventaris",
    Equipment: "Manajemen Jenis Peralatan",
    Packages: "Konfigurasi Paket Peralatan",
    InventoryList: "Inventarisasi Peralatan",
    StockIn: "Manajemen Masuk",
    MaterialRequestList: "Permintaan Materi",
    MaterialRequestDetail: "Detail Permintaan",
    IssueDraftList: "Konfirmasi Keluar Tertunda",
    IssueDraftDetail: "Konfirmasi Keluar",
    ManualStockOut: "Keluar Cepat (Tanpa Permintaan)",
    ReturnReceiving: "Penerimaan Kembali",
    StockFlowSettings: "Pengaturan Aliran Inventaris",
    StockHistory: "Catatan Masuk/Keluar",
    UserOwnership: "Buku Besar Kepemilikan Pengguna",
    Warehouses: "Manajemen Gudang",
    EquipmentInstances: "Daftar Contoh Peralatan",
    EquipmentLifecycle: "Siklus Hidup Peralatan",
    SitesMgmt: "Manajemen Situs",
    SiteList: "Daftar Situs",
    SiteMap: "Peta Situs",
    SitePlanningLldGlobal: "Ikhtisar Perencanaan LLD",
    SiteDetail: "Detail Situs",
    SiteSurveyStageBatch: "Pengaturan Batch Tahap Survei",
    SitePlanningLld: "Perencanaan Lokasi (LLD)",
    SiteBasicBatch: "Impor Info Situs",
    SitePlanningBatchLld: "Perencanaan Info Impor (LLD)",
    SiteBasicImportHistory: "Impor Riwayat",
    SurveyArchives: "Arsip Survei",
    SurveyArchiveDetail: "Detail Arsip",
    OpeningArchives: "Arsip Instalasi Situs",
    OpeningArchiveDetail: "Detail Arsip Instalasi Situs",
    SSVArchives: "Arsip SSV",
    SSVArchiveDetail: "Detail Arsip SSV",
    InspectionsMgmt: "Manajemen Inspeksi",
    InspectionTemplates: "Templat Inspeksi",
    TemplateEditor: "Penyunting Templat",
    WorkOrders: "Manajemen Perintah Kerja",
    MyExecutionWorkOrders: "Perintah Kerja Eksekusi Saya",
    WorkOrderList: "Daftar Perintah Kerja",
    WorkOrderReview: "Meja Peninjauan Perintah Kerja",
    WorkOrderPhotoDetail: "Detail Foto Serupa",
    WorkOrderExecute: "Meja Eksekusi Perintah Kerja",
    UsersMgmt: "Manajemen Pengguna",
    UserList: "Daftar Pengguna",
    Settings: "Pengaturan Sistem",
    MobileLocationSettings: "Konfigurasi Seluler",
    WorkOrderExecutionSettings: "Pengaturan Eksekusi Perintah Kerja Web",
    SitePaymentSettings: "Aturan Pembayaran Situs",
    GeocodeCache: "Cache Geokode",
    OmcConfig: "Konfigurasi API OMC",
    OmcDeviceStates: "Status Perangkat OMC",
    MockOmc: "Rintisan OMC Tiruan",
    DataBackup: "Cadangan Data",
    OperationLogs: "Log Operasi",
    MobileClientLogs: "Log Klien Seluler",
    AppVersionManage: "Manajemen Versi Aplikasi",
    ReleaseNotesPreview: "Catatan Rilis",
    AppVersionStats: "Statistik Penggunaan Aplikasi",
    AiManagement: "Manajemen AI",
    PermissionCenter: "Izin Peran",
  },
};
