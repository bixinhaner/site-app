...
const load = async (preserveEdits = false) => {
  try {
    loading.value = true
    const res = await openingArchivesApi.get(id)
    archive.value = res
...
async function exportZip() {
  try {
    ...
    const blob = await openingArchivesApi.exportZip(id)
...
async function exportPdf() {
  try {
    ...
    const blob = await openingArchivesApi.exportPdf(id)
    const url = window.URL.createObjectURL(new Blob([blob]))
    const a = document.createElement('a')
    a.href = url
...
function buildExportName(ext) {
  ...
  const raw = `开站档案_${code}_${name}_v${ver}_${tsStr}.${ext}`
  // 替换不安全字符
...
async function saveChanges() {
  try {
    ...
    await openingArchivesApi.patch(id, patchOps, archive.value.current_version, '前端编辑')
...
async function loadHistory() {
  try {
    ...
    const res = await openingArchivesApi.history(id)
...
async function onUploadPhoto(payload) {
  try {
    ...
    const res = await openingArchivesApi.uploadTempPhoto(id, {
...
async function onUploadAttachment(opt) {
  try {
    ...
    const file = opt?.file
    if (!file) return
    const res = await openingArchivesApi.uploadTempAttachment(id, { file })
    const att = {
      ...res,
...
function goBack() {
  router.push({ name: 'OpeningArchives' })
}
